...
**The coverage is proved rather than asserted, and the figure reproduces at `e41e72b25`, the commit this record is published at.** The tracked, existing, `oxfmt`-supported files outside `packages/` — `.md`, `.ts`, `.tsx`, `.json`, `.css`, `.html`, `.yml`, with `package-lock.json` refused by the tool as a lock file — number **121**, and the root leg of `just fmt` reports **121 files**; running `oxfmt --check` over exactly that enumerated list reports the same 121 and finds every one correctly formatted. Fifty-six of them are Markdown that no package recipe reaches. **Corrected 2026-09-12:** this pass first recorded 120 on both sides, a count taken mid-unit — before the two files the unit itself changes, a deleted `.prettierrc.json` and this record — so it was the figure of no commit. The method and the property are unchanged and the corrected pair is re-measured in a detached worktree at `e41e72b25`. No file is offered to two formatters, and the strong form of that is now structural: there is no second formatter in the repository.
