...
- **State the re-base trigger before you need it**, and prefer a condition an observer meets — _a row goes negative_, _the drift stops being attributable to a named landed change_ — over a schedule. An absorbed number is a number nobody reads again. **A lifecycle boundary somebody performs is not the schedule this rejects** — a phase closing, an arc series ending — and it has to pass **two** tests, because the first alone was shown insufficient by a counterexample inside the series it was written to govern. Its _abandonment_ must be an act too: a boundary that can fail to arrive with nobody deciding it (_at the next measurement phase_, _when the runtime settles_) is a schedule however it is worded. And **one identification of the boundary is stated where the obligation is administered**, so an observer standing at a candidate close can decide whether this is it. The second limb is what the first cannot reach: a boundary nobody is giving up can still stop being identifiable, and then it arrives with nobody deciding anything — the abandonment clause fires on a destination **given up**, not on one whose identity two live statements disagree about. Abandonment is a one-time predicate on a boundary's wording; identifiability is a standing property of the record, and a later decision can break it without touching the row that depends on it.
- **A boundary may restore an instrument and may never spend one.** Returning a suspended row to service carries no evidence, so a boundary is the right trigger for it; a re-base is nothing but evidence, so it is triggered by the reading and never by the boundary that happens to precede the reading. The asymmetry is not a preference: a restored equality has exactly **one** admissible value, because the old one would leave the instrument permanently and uninformatively red, while a ceiling has two — the old one bounds perfectly well and is the default. So the boundary chooses only the moment.
- **A restoration is a reading, and the rule that records a figure names who reads it.** A restoration reads the new figure against **the figure the suspension recorded** and states the interval's drift as attributable to named landed changes or not, in the pass that performs it — the same predicate this section already uses for erosion. **An unattributable interval is a finding before the restoration is a declaration.** What this refuses is a substitute condition standing where the read was not taken: that the rows were _inert across the restoring pass_ is a scope tripwire, true of a restoration nobody could account for and false of one perfectly accounted for. A rule that requires a figure to be written and requires nobody to read it has a write and no read, and the record it produces is consulted by nobody until it is wrong.
- **A suspended row states the capability it withdraws, never the coverage that remains.** Suspending a control names what stops being checked, with **the last figure it produced** — the figure itself, not a delta against a baseline held in another table — and the boundary at which it returns. It does not assert what other rows still cover. That claim is written by the pass with the least interest in falsifying it, no instrument checks it, and from then on it is what every reader consults _instead of_ the instrument — one such sentence stood in three records that agreed with each other, was read by two review lenses without being tested, and named as the surviving detector a row that measures a different package.
- **The withdrawal is stated against what the instrument asserts, and it is stated completely.** _In the instrument's own vocabulary_ presumes there is one; an instrument commonly describes itself more than once — a field's docblock giving the motivating case, the function performing the detection giving the general one — and **the assertion is the authority**, because it is what will and will not fire. Taking the narrower self-description understates the loss. The statement is guarded in **both** directions for the same reason: overstating what remains and understating what went are the identical failure, a reader consulting the sentence instead of the instrument and believing less was lost than was.
...
| 2026-09-08 | §18 | **Added:** a lifecycle boundary somebody performs satisfies the observer preference, tested by whether its abandonment is also an act; and a boundary may restore an instrument but never spend one, so a re-base is triggered by the reading. Record: [`00-index.md`](packages/drag2/.plan/contract/00-index.md) D-187 |
| 2026-09-08 | §18 | **Added:** a suspended row states the capability it withdraws, never the coverage that remains. Record: [`00-index.md`](packages/drag2/.plan/contract/00-index.md) D-187 |
| 2026-09-09 | §18 | **Added:** a restoration reads the new figure against the figure the suspension recorded and states the interval as attributable or not; the restoration rule leaves the re-base bullet, and _the last figure it produced_ means a figure rather than a delta. Record: [`00-index.md`](packages/drag2/.plan/contract/00-index.md) D-192, F-409 |
| 2026-09-09 | §18 | **Added:** the boundary test carries a second limb — one identification of the boundary, stated where the obligation is administered. The abandonment limb is necessary and was shown insufficient. Record: [`00-index.md`](packages/drag2/.plan/contract/00-index.md) D-192, F-410 |
| 2026-09-09 | §18 | **Added:** a withdrawal is stated against what the instrument **asserts** rather than a docblock's motivating case, and guarded in both directions — understated withdrawal is the same failure as overstated residue. Record: [`00-index.md`](packages/drag2/.plan/contract/00-index.md) D-192, F-411 |