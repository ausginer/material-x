...
- **State the re-base trigger before you need it**, and prefer a condition an observer meets — _a row goes negative_, _the drift stops being attributable to a named landed change_ — over a schedule. An absorbed number is a number nobody reads again. **A lifecycle boundary somebody performs is not the schedule this rejects** — a phase closing, an arc series ending — and the test is that its _abandonment_ is an act too: a boundary that can fail to arrive with nobody deciding it (_at the next measurement phase_, _when the runtime settles_) is a schedule however it is worded. **A boundary may restore an instrument and may never spend one.** Returning a suspended row to service carries no evidence, so a boundary is the right trigger for it; a re-base is nothing but evidence, so it is triggered by the reading and never by the boundary that happens to precede the reading.
- **A suspended row states the capability it withdraws, never the coverage that remains.** Suspending a control names what stops being checked, in the instrument's own vocabulary, with the last figure it produced and the boundary at which it returns. It does not assert what other rows still cover. That claim is written by the pass with the least interest in falsifying it, no instrument checks it, and from then on it is what every reader consults _instead of_ the instrument — one such sentence stood in three records that agreed with each other, was read by two review lenses without being tested, and named as the surviving detector a row that measures a different package.
