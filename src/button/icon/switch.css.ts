...
const mainStyles = TypedObject.entries(packs).flatMap(([s, packs]) => {
...
const widthStyles = TypedObject.entries(widthPacks).flatMap(([w, packs]) => {
  const width = attribute('width', w);

  return TypedObject.entries(packs).flatMap(([s, packs]) => {
