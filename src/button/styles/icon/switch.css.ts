...
const mainStyles = Object.entries(packs).flatMap(([s, packs]) => {
...
const widthStyles = Object.entries(widthPacks).flatMap(([w, packs]) => {
  const width = attribute('width', w);

  return Object.entries(packs).flatMap(([s, packs]) => {
