 ...
 *
 * This is workspace build tooling, not a ydin runtime export, and is not listed
 * in `files.json`. It runs `enforce: 'pre'` so it sees authored source before
 * other transforms. Each `impl` site is analyzed and lowered transactionally; a
 * failed proof warns and leaves the site unchanged.
 */
export function constructTraitFlattenerPlugin(): Rolldown.Plugin {
    ...
    async transform(code, id) {
      ...
      const loader: ModuleLoader = {
        resolve: async (specifier, importer) => {
          ...
          // External modules (e.g. the `ydin` package when building a
  ...
  // `enforce` is a Vite ordering hint; Rolldown ignores the extra field.
  (plugin as { enforce?: 'pre' }).enforce = 'pre';
