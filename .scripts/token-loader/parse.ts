...
import parseSingle from './parseSingle.ts';
...
async function parse(): Promise<void> {
  ...
  const processedTokens = unifier.tokens
    ...
    .map((token) => [token, parseSingle(token, unifier)] as const)
  ...
  await Promise.all([
    ...Object.entries(grouped)
      ...
      .map(async ([name, tokens]) => {
        const url = new URL(
          `${name.replaceAll('.', '-')}.json`,
          tokensCacheDir,
        );

        await writeFile(url, JSON.stringify([name, tokens], null, 2));
      }),
  ]);
}

await rm(tokensCacheDir, { recursive: true, force: true });
