...
**Two harness properties worth knowing.** A sub-agent inherits the full resident instruction set — `CLAUDE.md`, `AGENTS.md` and `CONTRIBUTING.md`, about 7,000 words — with no supported opt-out, so a four-lens round pays it four times; a dedicated launcher with an explicit system prompt is the remedy if that cost starts to bite. And the agent roster is read at session start, so a newly added role is invocable only after a restart.
