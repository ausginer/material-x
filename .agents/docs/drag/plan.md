...
14. **Every guarantee is rated at what the API provides, including the temporal
    ones** (C6-01). `write` is retained and stays effective, so the blanket
    late-use-safe claim is narrowed to the two capabilities that have it, and the
    lift is governed by a stated tier-C temporal rule instead — with the
    no-hot-path-guard choice recorded as a decision rather than an omission.
15. **State the fixture publishes, it also clears and defends** (C6-02):
    `retire()` drops `placeholder` and `lift` with `pendingRequest`, and the
    impossible missing-resource branch throws rather than returning, so the
    staged `ResolutionCommand` cannot execute for an operation whose
    presentation was never assembled.
