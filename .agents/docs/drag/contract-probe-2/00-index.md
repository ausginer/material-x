...
| F-4 | ~16 closures per controller from D-4 | **Accepted, measurement owed (M-2).** The earlier rejection of the opaque-`S` alternative — "it makes the kernel *store* behavior state" — does not survive scrutiny; holding an untyped reference neither knows nor widens it. The closure model ships for simplicity, not speed, and that has to be weighed. |
...
| F-24 | The hot-path accounting contradicted itself: "zero allocations, two indirect calls" versus a transform string and three calls | **Withdrawn and restated.** Three indirect calls, one string, plus a `{ x, y }` in the in-place lift mode. I-26 restated measurably; the falsifier that cited it is removed. |
