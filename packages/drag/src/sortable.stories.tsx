...
/**
 * A controlled sortable collection: the engine proposes a reorder through the
 * required, explicit `onReorder` resolution, React owns the order state and
 * commits it once the drop has settled (`onFinish`), and `updateItems` keeps the
 * engine in sync. Committing after settlement lets the landing animation reach
 * the destination slot before the list re-renders.
 */
function SortableDemo({
  ...
  useEffect(() => {
    ...
    // Predeclared so `commit` can close over it; assigned just below.
