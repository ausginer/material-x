...
| `accepted` is a trusted consumer guarantee that persistent authored presentation is ready before settlement; a resolution may instead carry an explicit `presentationReady` promise, and the temporary presentation is released only once landing has settled/skipped **and** readiness has resolved | required resolver contracts, landing targets, `watchPresentationReady`, feature settlement projections | accepted-ready ordering tests for free and sortable, plus barrier hold/release, rejected-acknowledgement, and timeout coverage |
...
Tests that encode the superseded contracts are removed rather than adapted: `undefined => accepted`, collection/model commit observation, DOM-order acknowledgement, accepted-but-uncommitted outcomes, and public programmatic movement. Their replacements are the explicit-resolution, dedicated-signal, currency, removal-precedence, authored-restoration, and presentation-failure cases above.

The bounded acknowledgement timeout is **not** among the superseded contracts. It was reinstated with the authored-presentation barrier below, in a different form: it no longer infers readiness from DOM or elapsed time, but bounds an explicit, consumer-supplied promise so a resolution that never settles cannot strand the gesture.
...
- `accepted` guarantees that the immutable proposal is accepted and persistent authored presentation is ready for settlement and eventual restoration. The library trusts rather than verifies this guarantee, but a consumer that cannot make it synchronously may instead defer it explicitly — see the authored-presentation barrier below.
...
### Final decision: the authored-presentation barrier

`accepted` alone proved insufficient for consumers whose authored presentation is committed asynchronously. A React consumer calls `setState` from `onReorder` and cannot truthfully claim its DOM is ready when it returns; the render lands a frame or more later. The original contract assumed the landing animation would cover that gap, which is a race — and one that is already lost under `prefers-reduced-motion`, where recovery is `RECOVERY_IMMEDIATE` -> `LANDING_SKIPPED` and settlement completes synchronously, and for every accepted *free* drop, which always takes `RECOVERY_IMMEDIATE` and so never had a landing to hide behind.

Both `ReorderResolution` and `FreeDropResolution` therefore accept an optional `presentationReady: PromiseLike<void>` on accepted **and** rejected results, through `accept(presentationReady?)` and `reject(reason?, presentationReady?)`. Release of the temporary presentation is gated on both halves of a barrier — `canReleasePresentation`:

```
landing finished or skipped   AND   authored presentation ready
```

The two run **concurrently**. This is the reason the readiness signal is a separate field rather than an `await` inside the resolver: landing is only prepared after the resolution settles, so awaiting the commit before returning `accepted` serialises the consumer's render ahead of the animation and makes every drop feel laggy. The separate field is what buys the overlap, and it must not later be "simplified" away.

Ownership stays with the consumer. The kernel never observes slots, mutations, collections, or framework state; `watchPresentationReady` owns only the waiting — `ResolutionCurrency` tagging, so a promise from an abandoned gesture cannot resolve into the next operation, and a bounded `PRESENTATION_READY_TIMEOUT`. A rejected or timed-out acknowledgement is a concrete `presentation-ready` failure reported through `onError`, after which recovery switches to home, since the destination authored presentation cannot be assumed to exist. Cleanup always runs: a late authored render is a visual glitch, a stranded gesture is a broken component.

Optimistically mutating authored presentation during an active gesture remains outside the contract. The rejected result supports an explicit async rollback barrier; cancellation paths that carry no resolution introduce no further readiness hook.
