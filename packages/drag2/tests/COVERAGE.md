...
| `current.operation === pinned` dropped | **none** — and _none could be written_ was wrong, see below |

~~**The third conjunct has no reachable falsifier, so `#pinned` did not move.**~~ **The pin did not move, and the reason recorded here was wrong** (D-185, 2026-09-07). Three of the four clauses hold: `#retireOperation` nulls `current.operation` and clears `#pinned` with nothing between them, `cancel()` is caught by the second conjunct and `destroy()` by the first. The fourth — _the only other writer of `current.operation` is an admission the execution bracket refuses reentrantly_ — does not: [`execution.ts`](../src/kernel/execution.ts)'s `runIngress` refuses on `#closed` and `#admitting`, so it refuses a pass raised from inside an **admission** and none raised from inside a **seam phase**.

**A row can be written, and this is its construction.** `#handleBehaviorAction` is the one seam that runs with no live operation, so `#openIngress`'s `current.operation` guard admits through it. Dispatch a behavior action while idle from a behavior whose `command.types` carry an event its own `action.prepare` can fire synchronously on `root` — `['focusin']` and an `item.focus()` is enough. The nested pass re-pins, rebuilds the draft the outer `prepare` holds, and commits a new operation; the outer revalidation then compares against a pin taken before that operation existed. **With the conjunct the outer seam rolls back and the nested operation completes; without it the outer seam commits over the nested operation's frame, and that operation is minted, armed, never activated and never retired, with nothing reported.** The mutation is therefore not equivalent, and the conjunct does not belong in §Equivalent mutants.

**Why the suite is nonetheless green under the mutation.** No row drives a nested ingress pass from inside a seam. That is the gap, and it is Q-27's — the refusal the ingress path is missing is the subject, and the conjunct is a holding guard until it is answered.
