...
| `current.operation === pinned` dropped | **none, and none could be written** — see the reasoning below, repaired |

**The third conjunct has no reachable falsifier, so `#pinned` did not move** — and the reason recorded here was wrong in one clause, repaired 2026-09-07 (D-186). Three clauses hold: `#retireOperation` nulls `current.operation` and clears `#pinned` with nothing between them, `cancel()` is caught by the second conjunct and `destroy()` by the first. ~~The only other writer of `current.operation` is an admission the execution bracket refuses reentrantly.~~ **Retired: [`execution.ts`](../src/kernel/execution.ts)'s `runIngress` refuses on `#closed` and `#admitting`, which spans an admission and not a seam phase.**
...
**So the entry stays in this table and the mutation stays equivalent** over every path correct use of the contract reaches, which is what this register records. Relocating a guard into a new entity in the same commit that would justify deleting it makes the deletion unreviewable afterwards, so the field stayed where it was and the arc recorded the finding instead; D-186 deletes it.
