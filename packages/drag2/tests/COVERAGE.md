...
| `arm()` rejects a `pointerdown`-colliding `command.types`, and that is the only shape it rejects | `tests/kernel/kernel.browser.test.ts` — _should reject a command.types entry colliding with the pointer ingress_ | D-32, D-118 |
| an **empty** `types` array arms and binds no discrete listener, call-for-call identical to a spec declaring no `command` member | `tests/kernel/kernel.browser.test.ts` — _should arm an empty command.types as the no-command configuration_ | D-118 |
| an **empty-string** entry arms and binds an ordinary, distinct listener that no other event reaches | `tests/kernel/kernel.browser.test.ts` — _should arm a command.types carrying an empty-string entry_ | D-118 |
| a **duplicate** entry arms instead, because `addEventListener` already dedups on (type, callback, capture) | `tests/kernel/kernel.browser.test.ts` — _should arm a command.types carrying a duplicate entry_ | D-32 |
...
| ~~late landing completion~~ | **Nothing completes (D-155)**, so a late completion has no producer and the row has no subject. ~~The staleness the row pinned is now answered by identity:~~ **corrected 2026-09-08 (F-401)** — the row named here, `tests/kernel/kernel.browser.test.ts` — _should replace an open settlement without disturbing a live tail_ — asserts a queued failure's stage and a non-idle tail animation, and no identity at all; the settlement identity it was said to be answered by is deleted with D-189 | I-24 |
...
| `invalidate()` after `destroy()` does not reach the consumer's own `items()` at all — the one piece of consumer code on the path, and what the pre-D3 order ran before the closed latch (re-pointed from the retired duplicate refusal, D-121) | `tests/sortable/sortable.browser.test.ts` — _should not pull the collection at all for a post-destroy replacement_ | D3, D-121 |
...
| one throwing retire hook does not prevent the rest, reverse order | `tests/sortable/sortable.browser.test.ts` — _should run the retire hooks, each wrapped_; `tests/sortable/assemble.browser.test.ts` — _should expose retire hooks in installation order_ | F-22 |
...
| the probe that found it now compiles as a positive | `tests/probes/13c-free-drag.ts` — `n1`, asserted by `npx just typecheck` | F-44 |
...
| a marker or a deferred row that does not parse is a failure, and `\| D-nn \|` outside the section is not one. **Inside the section the subject is the table rather than the shape of a row**: any line beginning `\|` that is neither the delimiter, nor the header the delimiter follows, nor `LISTED`-parseable is a failure, so a one-cell row of prose cannot fall through | `tests/decisions.node.test.ts` — _should refuse a marker it cannot parse rather than skipping it_, _should refuse a table row it cannot parse rather than skipping it_, _should refuse a row of prose that carries no decision at all_, _should read the header off the delimiter rather than off its text_, _should not read the decision tables as deferred rows_ | F-70, F-414 |
...
| the proposal builder's **version** test stays — a gap describing a snapshot that is gone is arithmetic no author can satisfy | `tests/sortable/sortable.browser.test.ts` — _should still refuse an insertion carrying another version_ | I-12, D-123 |
| ~~its **neighbour-coherence** and **range** tests~~ — **deleted D-123**; `insertionAt` is published from `sortable/feature.js`, so the term is satisfiable and a disagreeing gap is a non-conforming contribution. The request carries the author's own `before`/`after` onward, and the destination view is no longer materialized at all | `tests/sortable/sortable.browser.test.ts` — _should not refuse an insertion whose neighbours the snapshot does not support_ | D-119, D-123 |
...
| a rejection value of **`null`** is an ordinary settlement failure and not the driver's re-entry escape — same stage, `cause === null`, terminal published, controller still admitting | `tests/sortable/sortable.browser.test.ts` — _should classify a null rejection rather than reading it as a re-entry_ | F-170, D-153 |
...
| Row | Test | ID |
| --- | --- | --- |
| each seam calls `kernel.fail` and returns normally → no success continuation | `tests/kernel/seams.node.test.ts` — _should treat kernel.fail in prepare as a prepare failure_, _should not run the effect after a latched prepare failure_, _should never invoke the consumer after a latched release failure_, _should not dispatch the checkpoint on a latched effect failure_ | F-34 |
...
| the library's own `<div>` records **no** rollback, and no longer takes the snapshot it would discard | `tests/sortable/placement.browser.test.ts` — _should take no rollback snapshot for the default placeholder_ | D-39, D-127 |
...
| …but **not** when a presentation disposer destroyed the controller on the way: the reporting route reads the latch between the dispose and the terminal | `tests/kernel/kernel.browser.test.ts` — _should publish no terminal once a presentation disposer destroyed the controller on the error route_ | I-36, D-179, F-328 |
| …and the route still retires its own operation, rather than leaving it to the deferred teardown. **Booked to the ordered tail rather than to `toContain('retire')`**, which cannot fail: the destroy path calls `spec.retire` itself, so a presence assertion reads that call and stays green with the retirement conditioned. What discriminates is the count — the lifetimes self-guard, `BehaviorSpec.retire` is _idempotent, best-effort_ and the kernel keeps no flag, so it is the one step that runs once per retirement | `tests/kernel/kernel.browser.test.ts` — the same row, asserting `presentation.released, retire, retire` | D-179, F-328 |
...
| Row | Test | ID |
| --- | --- | --- |
| `AxisInstaller` is **hoistable** from `sortable.js` by a consumer importing nothing deeper, while its closure stays at the middle tier | `tests/consumer.node.test.ts` — _should compile a consumer against the packed declarations_ (the `hoistedAxis` const and the surviving `@ts-expect-error`s on `AxisContribution`/`InsertionGeometry`/`FeatureContext`) | D-78 |
...
| ~~a duplicated element is refused **before any installer runs**~~ — **deleted D-121**; the refusal it ordered no longer exists, and D-80 (b)'s ordering survives on F-69's reason alone (F-98) | — | D-80 (b), F-68, D-121 |
...
| Row | Test | ID |
| --- | --- | --- |
| ~~a landing runner failing after the commit keeps `accepted`~~ / ~~…and so does a runner that cannot be created~~ | **Both producers retire with the runner (D-155)**, and the pin with D-166, so the join contributes no post-commit stage of its own. The two that remain are the behavior tier's — a `settlement.effect` classified from the committed state, where the lookup decides the operation's only terminal, and a throwing terminal callback, which is what the surviving rows drive: `tests/sortable/sortable.browser.test.ts` — _should keep the accepted result when the terminal callback throws_, _should keep a rejected result too, not just an accepted one_ | D-66, A-1 |
...
**The rejection row asserts the consequence, not the guard.** What a consumer sees if the ignoring is ever done by declining to subscribe is an `unhandledrejection` in their console. Falsified by dropping the rejection handler from the kernel's `then.call`: two rows fail, this one among them.

~~Every other row about an abandoned resolver asserts the slot comparison `resolution !== attempt`, which is how the library _ignores_ a stale settlement.~~ **False, and corrected 2026-09-08 (F-400).** Executed at `a5b11c09e`, deleting either `#attempts.resolution !== attempt` conjunct — or both together — left every project green: no row anywhere asserted the comparison, in either half of I-4's double validation. What the sentence was contrasting this row against did not exist. The two rows that assert it now are [§I-4's double validation, instrumented](#i-4s-double-validation-instrumented--new-2026-09-08-f-400), and the contrast holds against those.
...
**The three runtime rows above have no type-level half, and the declaration rows are it.** The reflection over `FAILURE_*` cannot see `FailureStage`, and `'code' in error` is a property of one constructed instance: a `code?: string` re-declared on the class, or a `FailureStage` widened to `number`, passes every one of them and reaches a consumer's compiler as the first thing that notices. Falsified by either edit — the union row fails on the widening and the `@ts-expect-error` on the re-declaration, which is the direction that matters, since an unused `@ts-expect-error` is itself a type error.
...
| a custom-element placeholder's `disconnectedCallback` destroys during a committed `movePlaceholder` → **nothing after the reaction runs**, counted as the displacement report the axis never reaches: `applied` is empty | `tests/sortable/features.browser.test.ts` — _should not run the bracket past a placeholder reaction that destroyed_ | I-36 |
| **`settle` is a declared consumer slot** (D-176): a `box` resolver that destroys on the **last** candidate — where the scan completes and no further `box` is owed a reading — leaves the sink uncalled through `RectIndex.refresh`, and the same close raised on the probe leaves it uncalled through `LinearShift.moved`, which takes `#drop()` instead | `tests/sortable/y.browser.test.ts` — _should invoke no settle once the last candidate closed the controller_, _should invoke no settle from a committed move once the probe read closed the controller_ | I-36, D-176, F-312 |
| the two positive controls, because a barrier that never lets the sink run is indistinguishable from one that works: a surviving rebuild settles **once**, and a dropped prediction makes the next rebuild measure rather than serve the warm buffer | `tests/sortable/y.browser.test.ts` — _should invoke settle once on a rebuild the controller survived_, _should forget the prediction when the probe read closed the controller_ | I-36, D-176, F-312 |
| **`invalidateInsertion` is one too**, and the committed-move bracket's `finally` is where it is invoked after a close: middle-tier `moved` that destroys and then throws leaves the axis uninvalidated, because teardown retires its cache through `retireHooks` | `tests/sortable/activation-barrier.browser.test.ts` — _should not invalidate the axis once the committed move destroyed the controller_ | I-36, D-176, F-324 |
| the control beside it: the same hostile `moved` **without** the destroy still invalidates, so the barrier cannot pass by swallowing the staleness repair | `tests/sortable/activation-barrier.browser.test.ts` — _should invalidate the axis when the committed move failed without closing_ | I-36, F-324 |

### The indirect half — new (Checkpoint D review 4, C4-01)

> I-36's indirect-invocation clause reaches DOM methods the library calls on consumer-owned nodes and not only the named resolver slots. **The whole-program ceiling that phrasing implied is withdrawn**: the obligation is over **declared consumer slots**, and one latch reading sits immediately before each `box` invocation and nowhere else — so a composition naming neither `box` nor `visual` takes no reading in this loop at all. `getBoundingClientRect` and `compareDocumentPosition` on a consumer-owned node are platform members, not declared slots.

**The discriminating candidate is any one with another after it.** A close raised from a candidate's own geometry read is caught by the reading before the **next** invocation, which no reading placed after a call could see. On the **last** candidate nothing is owed, so the rebuild completes — the three rows that pinned the opposite are retargeted here onto the completion, and are marked as such. The final row is a conformance pin added at review 4 and is labelled as one: it passes against current source, guards no reading, and pins the size of a residue I-36 (2) classifies as conforming. It was verified to be **sensitive** rather than vacuous by removing the kernel's F-30 handle disposal, against which it fails.

| Property | Where | Invariant |
| --- | --- | --- |
| **retargeted** — a candidate's own `getBoundingClientRect()` destroys with **no `visual()` composed**, and nothing is owed after the last candidate: the rebuild completes and the placeholder is read **exactly once**. A composition with no resolver invokes no declared slot in this loop | `tests/sortable/{y,xy}.browser.test.ts` — _should still read the placeholder once the last candidate closed the controller_ | I-36 |
| **retargeted** — the same → the buffer is whole, so the cache is left **clean** at that version and the same version finds it warm. The rows are released by the transaction boundary that runs the retire hooks | `tests/sortable/{y,xy}.browser.test.ts` — _should leave the cache clean after the last candidate closed the controller_ | I-36, I-20 |
...
| a `refresh` entered already closed calls the resolver **zero** times — the release path, where the invalidation runs immediately before `resolveInsertion`. Carried by the **first candidate's own** reading now that the separate entry reading has gone | `tests/sortable/{y,xy}.browser.test.ts` — _should call no resolver at all when the controller is already closed_ | I-36 |
| **retargeted, `xy()` only** — a close raised from the placeholder's anchor rect does **not** stop `compareDocumentPosition`: it is a platform query on a consumer-owned node, and the insertion it feeds is refused by the behavior and again by the kernel | `tests/sortable/xy.browser.test.ts` — _should compare document position even after the anchor read closed the controller_ | I-36 |
| the sink measures nothing at all, so its only consumer-reaching call is `animate()` on a row: a controller closed before or during the walk starts **zero** contributions | `tests/sortable/displacement.browser.test.ts` — _should start no contribution for a row once the sink is closed_ | I-36 |
...
| **the stretch sweep** — the release frame writes: an axis that destroys while resolving the release leaves `draft.proposal` null | `tests/sortable/sortable.browser.test.ts` — _should build no proposal when the axis destroys the controller while resolving the release_ | I-36 (2), I-20 |
...
| `phase !== RELEASING` in `#handleResolutionSettled` (`src/kernel/kernel.ts`) | The slot comparison beside it covers every path that exists today, because everything that decides the operation also clears the slot. Dropping it reddens **0** of the 1,366 rows, against **1** for the slot comparison it stands behind (2026-09-08, F-400). It is the second layer I-4 asks for, guarding the window where slot and phase disagree, and it stays for the reason this table exists. |
...
| the round-trip opens on release and publishes **exactly one** terminal, for accept, reject and async accept | _should open the round-trip on release and publish one terminal_, _should publish a rejected terminal for a rejected drop_, _should await an async acceptance_ | D-62, D-66 |
...
| an axis scalar locks the cross axis. ~~a source is read at activation and on `invalidate()`, never per sample~~ — **`AxisSource` is deleted** (D-148): an axis is applied to travel already accumulated, so a live one reinterprets every pixel since the grab rather than changing what happens next, and the per-sample case it served is a `bounds` installer | _should lock the cross axis when a scalar names one_ | D-71, D-148 |
...
| an unknown `lift` value completes a normal drag — a JS consumer's only route to one since D-141 gave the slot the kernel's numeric union | `tests/free-drag/validation.browser.test.ts` — _should complete a normal drag for an unknown lift string_ | B-4 (c), D-141 |
...
| the driver's non-classifying routes are one collector: a throwing `rollback`, a `kernel.fail` inside one, a `kernel.fail` outside a seam, a phase that failed then threw, an unconsumed staged value | `tests/kernel/seams.node.test.ts` — _should report kernel.fail outside a seam as a warning rather than classifying it_, _should classify only once when a phase latches a failure and then throws_, _should report a throwing rollback without classifying it_, _should report kernel.fail inside rollback exactly like a throw inside it_ | D-130 §5 |
...
| the constructed message carries the stage **number**, the `null` message is a fixed string, and the caught error's message still wins | `tests/kernel/errors.node.test.ts` — _should carry the stage number in the message it builds itself_, _should say the controller is destroyed when there is no stage_, _should prefer the classifying error message over its own_ | D-133, F-104 |
...
**Amended by D-133, and the amendment removed a row rather than adding one.** `STAGE_NAMES` — twelve stages rendered in words for the constructed message — is deleted, and the row that read a neighbouring stage's rendered name goes with it. **That row was the last positional witness in the library**, and losing it strengthens rather than weakens the 12/13 claim: a padded array only ever caught a hole being closed up, while the reflection in `stages.node.test.ts` catches that _and_ a constant reintroduced at either number, which is the failure the never-reuse rule actually forbids. What the deleted table was for never worked — the fallback fires only on the non-`Error` path, so the logged payload D-132 §5.3 set out to improve never saw a word (F-105).

**The compiled fixture is a coverage row in its own right.** `tests/revision/revision-2.ts`'s `report()` used to be `error.code === 'consumer' ? 'mine' : 'theirs'` and answered _theirs_ four ways for a fault entirely the consumer's; it is now three tests over the stages that name a caller, plus the `null` arm. `n8` inverted with it, from `@ts-expect-error a FailureStage is not a DraggableErrorCode` into a positive assignment — the negative had nothing left to refuse once there was one vocabulary instead of two.
...
| Row | Test | ID |
| --- | --- | --- |
| `faithful` keeps the stage geometry and travels by the **viewport** delta, under a transform and under zoom | `tests/free-drag/geometry.browser.test.ts` — _should keep the stage geometry and travel by the viewport delta under a transform_, _should keep the stage geometry and travel by the viewport delta under zoom_ | L-5, D-141 |
...
| the local delta is the viewport delta in the stage's own space, and is the delta itself when the ancestry is untransformed | _should report the local delta in the stage space under a transform_, _should report the viewport delta itself when the ancestry is untransformed_ | D-72 |
...
| box-quad's specifier appears in exactly one `src/` module, the kernel seam | `tests/packaging.node.test.ts` — _should import the geometry package in exactly one module_ | D-85 |
...
| an invalid `home` result is attributed to its own quality seam — `null` and a throwing accessor. A **non-finite** one is no longer refused (D-124) and composes into the target unattributed | `tests/free-drag/validation.browser.test.ts` — _should attribute a null result to the landing target seam_, _should let a non-finite result compose into the target unattributed_, _should attribute a result with a throwing accessor to the same seam_ | E-05, D-49, D-124 |
...
| cross-behavior installers do not compile in **either** direction, `AxisInstaller → FreeDragPlugin` named explicitly | `tests/composition.declaration.test.ts` — _should not be assignable to a free-drag installer_, _should not be assignable to a free-drag plugin_, _should not be assignable to an axis installer_ | E-06, D-87, D-138 |
| the boundary itself: the two **branded contexts** are mutually unassignable, and each is still the shared context plus one brand | _should not be mutually assignable_, _should each still be the shared context plus the brand_ | D-138, F-64, B-7 |
| an installer that declares no context **stays** assignable to both — the accepted hole, asserted so it is a decision rather than a gap | _should stay assignable to both behaviors when it declares no context_ | D-138 |
...
**The first row is the one to read**: D-87's mechanism was recorded as sufficient and was not, and the suite that was supposed to evidence it could not see the gap — every row started from an annotated installer **alias**, which is the one form the two exclusions already caught. What escaped was the unannotated hoisted literal D-78 says an ordinary author writes, refused not by the exclusions but by TypeScript's weak-type check, which `retire` defeats.

**D-138 replaced the mechanism and the unannotated rows went with it**, deliberately. Key-set totality is what made each record enumerate the other behavior's capability names, and independence is the property the owner asked for — so the escaped-form coverage is not a regression against D-88 but the price of the requirement that replaced it, recorded in the decision and asserted as the accepted hole rather than left to be rediscovered. The surviving rows are the four alias directions, the brand at its source, and the shared-slot positive control.

**D-91 was falsified against the pre-fix tree**; removing the finiteness check fails the two `moveTo` rows. D-89 needed its fixture rebuilt before it discriminated at all — see the row's own note. **D-138 is falsified by deleting either brand**: the two branded-context rows fail first and the four installer rows second, which is the ordering that says the brand and not a slot shape is carrying the boundary.

| Row | Test | ID |
| --- | --- | --- |
| the behaviors' contribution groups declare **independent** key sets, neither naming the other's vocabulary — and since D-146 neither plugin group declares a unique slot at all, which is the cardinality rule stated over both behaviors at once | `tests/composition.declaration.test.ts` — _should declare independent key sets_, _should give the unbounded position no unique slot_ | D-138, CE1-01, D-146 |
| a third-party installer is typed by the slot it fills or by its alias, and never writes the brand | _should type a plugin parameter from the slot it fills_, _should type a hoisted installer from its alias_ | D-138, D-78 |
| an installer carrying only the genuinely shared vocabulary reaches free drag's public `landing` key — the control that keeps the boundary from being read as a general separation. **It moved off `plugins` at D-146**, because the unbounded position no longer reaches a unique slot | _should still reach the landing key with only shared vocabulary_ | D-88, D-138, D-146 |
...
| each of the constraint's **five** call sites is driven **alone** and is never handed the constraint record, so re-attaching any one site fails one row — the fixture records the `this` it is given instead of ignoring it, which is what the reverted row could not do | _should hand the apply site a foreign receiver_, _should hand the scroll invalidator a foreign receiver_, _should hand the policy invalidator a foreign receiver_, _should hand the retire hook a foreign receiver_ | D-90, CE1-03 |
| the fifth site is the **construction unwind**, reached when a later installer throws — the site where `=== undefined` was false, since `retireHooks[i]!()` hands the hook the internal array | _should hand the construction unwind a foreign receiver_ | D-93, CE4-01 |
...
| ~~a non-finite `moveTo()` writes nothing into committed frame state and surfaces on the platform channel~~ — **deleted D-124**; it is written, poisons the derived geometry, and reports nothing | _should write the non-finite offset into the committed frame_, _should surface nothing on the platform channel_ | D-91, CE1-04, D-124 |
...
| **the coupling, executed rather than traced** (D-124): a non-finite `moveTo()` reaches the library-minted `LandingTimingContext.distance` a conforming `duration` thunk is called with, and goes on into the landing unrefused at either end. ~~Two further rows asserted what the compounded value then does, and what `NaN` does instead~~ — **deleted 2026-08-25** (the landing review §1.1 (C)): one froze the shape of undefined behaviour, the other pinned Chrome's `animate()` domain, which `.plan/measurements/animate-duration-domain.md` already owns | `tests/free-drag/validation.browser.test.ts` — _should mint a non-finite distance for a conforming duration thunk_, _should carry the minted distance into the landing unrefused_ | D-124 |
...
**Counts corrected to five sites in both tiers on 2026-08-19** (D-93), and the assertion form is `receiver !== own()` in both. `=== undefined` was a **mechanism** claim: it held at four sites per tier and was false at the construction unwind, where an indexed `retireHooks[i]!()` hands the hook the assembler's internal array. The obligation held at every site throughout — which is why the obligation is what both instruments assert.
...
**Both tiers assert the same thing: the receiver is never the capability record the member is declared on** — the nested `InsertionGeometry` or `MotionConstraint`, not the contribution object carrying it (D-94). That referent is what makes the rows discriminating: a bound `contribution.insertion.resolve(…)` never uses the contribution object either, so a row written against that object would pass on the tree the convention forbids. The single negative is the whole of the published obligation and every other receiver value is unspecified, so no row in either instrument names one, and none can be broken by a refactor that leaves the convention intact. `tests/free-drag/anchor.browser.test.ts` pins free drag's half in the identical form.

| Row | Test | ID |
| --- | --- | --- |
| each of the four lifted members is driven **alone** and is never handed the nested geometry it is declared on, so re-binding a single lift in `assemble` fails a single row | _should hand the resolve site a foreign receiver_, _should hand the invalidate site a foreign receiver_, _should hand the moved site a foreign receiver_, _should hand the retire site a foreign receiver_ | D-92 |
...
| no comment in `src/` carries a strikethrough, a date or a phase number, so a register that may argue for what is cannot narrate what was — and the list stops there, because a decision pointer is permitted here and a supersession verb is ambiguous in present-tense prose | `tests/references.node.test.ts` — _should carry no history in a comment_ | D-137, F-111 |
...
| the published declarations carry no doc block with no subject, and no orphan is exempt — D-135 emptied the one exempt class by forbidding a strikethrough in a published declaration at all | `tests/packaging.node.test.ts` — _should publish no doc block that documents nothing_ | D-113, D-135, MNT-01, MNT-02 |
| nothing the tarball carries names a decision, a finding, a contract section, the size policy, the record, a phase, a date or a struck sentence, so a consumer's editor is never shown a reference it cannot follow | `tests/packaging.node.test.ts` — _should publish no declaration carrying an internal reference_ | D-135, F-107 |
...
| no surviving table row authors more cells than its header declares, so a clause cannot exist where it is written and vanish where it is read — and a whole entry hidden behind the third cell is reported apart from a truncated clause, because the repairs differ | `tests/decisions.node.test.ts` — _should find no surplus cell anywhere in the record_, _should find a clause truncated off the end of a row_, _should find a whole entry hidden behind the last rendered cell_, _should keep counting against the header across a blank line_ | F-83, F-284 |
...
| an identifier opening a heading is an ownership claim at **any** depth, so a restatement below the entry depth is reported rather than stepped over | `tests/decisions.node.test.ts` — _should read a claim at any depth, not only at the entry depth_, _should report a claim that sits off the entry depth_, _should accept a claim at the entry depth_, _should not read a heading that merely mentions an identifier as a claim_ | D-174, F-287 |
| no identifier is claimed twice across the current-state tree, and the report names both sites — **including twice inside one document**, which is the shape a canonical-id collision in a register takes, since a register is one file — the ownership map is keyed across documents rather than within one | `tests/decisions.node.test.ts` — _should report one identifier claimed by two documents_, _should report one identifier claimed twice in the same document_, _should hold the invariant across every current-state document_, _should read enough of the tree for that to mean something_ | D-174, D-196, F-287 |
| `ID §…` is a named sub-clause rather than a claim: legal below `####`, and required to nest under the entry claiming that same identifier | `tests/decisions.node.test.ts` — _should accept a sub-clause nested under the entry claiming it_, _should report a sub-clause filed under another entry_, _should report a sub-clause that sits at claim depth_, _should report an identifier heading in neither form_ | D-175 |
| one identifier grammar with no letter budget, so a four-character family is read rather than silently unmatched | `tests/decisions.node.test.ts` — _should read an identifier of more than three prefix characters_; `tests/entry.node.test.ts` — _should resolve a multi-letter family through the same syntax_ | D-175, F-289 |
| no link addresses a canonical entry through a generated heading fragment, and a quoted specimen of one is not read as a link | `tests/decisions.node.test.ts` — _should target no canonical entry through a heading fragment_, _should catch the form F-288 records_, _should still permit a fragment against an identifier-free section_, _should not read a quoted specimen as a link_ | D-175, F-288 |
| the reader's duplicate and the integrity layer's are different reports of different things, and only the second sees a claim the extractor never reads | `tests/entry.node.test.ts` — _should keep answering where only the integrity layer sees a duplicate_, _should refuse a local id the record defines twice_ | D-175, F-287 |
| every identifier family the record defines resolves through the qualified address | `tests/entry.node.test.ts` — _should resolve an address in every family the record defines_ | D-173, D-175 |

## The free-drag owner-review cleanup — new (2026-08-26, D-139…D-142)

**Four of these rows exist because something was deleted, and one because something got cheaper.** The suite lost exactly one row — the one that drove an `onDrop` returning `42` and expected a classified failure — which asserted the duck-type gate D-140 deletes and nothing else. Nothing replaces it, because what it tested is now unreachable through the types and the contract says so rather than checking it.

| Row | Test | ID |
| --- | --- | --- |
| acceptance is **one shared value**, so an accepted drop allocates nothing, and identity is what tells the two arms apart | `tests/free-drag/domain.node.test.ts` — _should return one shared value for every acceptance_ | D-140 |
| a rejection carries its reason and stays distinct from acceptance, **including with no reason at all** — the argument is optional and omitting it must not collapse the arms | _should carry the reason on a rejection and stay distinct from acceptance_, _should build a rejection with no reason rather than an acceptance_ | D-140 |
| the resolution is opaque from the consumer's side: no discriminant to read, no literal to forge, and neither arm importable | `tests/consumer.node.test.ts` — the `@ts-expect-error` rows on `accepted.type`, on a `{ type: 'accepted' }` literal and on `AcceptedFreeDragResolution` | D-140 |
| `config.lift` takes the kernel's numeric mode and no string is one, while the three constants reach an ordinary consumer **from `free-drag.js`** | `tests/consumer.node.test.ts` — the `liftModes` const and the `@ts-expect-error` on `lift: 'top-layer'`; _should expose exactly the intended runtime surface, per subpath_ (seven names); `tests/exports.node.test.ts` — _should export exactly the frozen runtime surface_ | D-141 |
| the geometry's coordinates are scalars, and the request's with them — **all five retired pairs refused, and all sixteen surviving fields read by name** (F-120), because a flattening fails by transposing an X for a Y and a fixture reading one field of eight cannot see that | `tests/consumer.node.test.ts` — the five `@ts-expect-error` rows on `viewportDelta`, `viewportPosition`, `localDelta`, `pointer` and `originPointer`; the coordinate reads inside `onDrop`, `onStart` and `onMove`; the free-drag geometry and activation-snapshot suites, re-pointed field by field | D-139, F-120 |
...
## The owner-review remediation — new (2026-08-27, F-117…F-121)

**Every row here closes a gap the review found in this file's own subject: what is pinned, and what only looked pinned.** Three of the five findings were prose; these are the two that were coverage.

| Row | Test | ID |
| --- | --- | --- |
| a contribution typed for the other behavior is refused **through the public `plugins` slot**, which is the position CE1-01's silent discard was reachable from — and the boundary is on the installer's context, so a foreign parameter annotation is what it refuses | `tests/composition.declaration.test.ts` — _should refuse a hoisted contribution typed for the other behavior_ | D-138, F-117 |
| its **control**, and the row above means nothing without it: the same contribution carrying only the shared `retire` slot **compiles** when the context is its own, so the negative cannot be passing on the contribution's shape (F-74's lesson, which outlived the mechanism that taught it) | _should accept the same contribution when the context is its own_ | F-74, F-117 |
| neither behavior publishes its internal resolution arms, on either entry — the names are erased, so a value-equality export assertion cannot see one joining a re-export list | `tests/consumer.node.test.ts` — the four `@ts-expect-error` rows on `AcceptedResolution`/`RejectedResolution` | D-140, D-143, F-120 |
| `ItemSource`, `ResolveHandle` and `ResolveElement` are **hoistable from `sortable.js`**, asserted in a type position rather than in a JSDoc paragraph — deleting the three re-exports used to leave the suite green | `tests/consumer.node.test.ts` — the three annotated `const`s below the sortable fixture's opacity heading | D-78, F-120 |

## The borrowed anchor and the flattened landing chain — new (2026-08-27, D-144, D-145)

**One of these rows is a contract that was true and unstated, and the rest are the surface following the shape.** `BehaviorSpec.anchorTarget` never said what the kernel does with the point it returns; the kernel read both fields immediately and kept nothing, which is what made a per-controller buffer safe, and nothing anywhere asserted it (F-123). A future edit that stored the object on the settlement attempt would have broken every caching behavior silently and passed the whole suite. The landing chain's rows are the other half: four nested coordinates became scalars, and the consumer-facing witness for them is a `duration` function, which is the only place a consumer meets the landing's endpoints at all.
...
## Per-key contribution groups — new (2026-08-27, D-146)

**Three runtime rows were deleted and their property moved one tier up, which is the whole shape of this slice.** `claim` arbitrated a collision at construction time; the groups make the collision unwritable, so the compositions those rows built no longer typecheck and there is no runtime behavior left to assert. Each replacement carries the F-74 control beside it, because a negative that passes on a literal's shape rather than on a slot's absence from a group is indistinguishable from a working boundary.

| Row | Test | ID |
| --- | --- | --- |
| the `axis` key's group **requires** the geometry it exists to install, which is both the replacement for the deleted construction-time check and the whole of the slot's cardinality | `tests/sortable/feature.declaration.test.ts` — _should require the geometry it exists to install_ | D-146, D-77 |
| the unbounded position reaches **no** unique slot in either behavior, because neither plugin group declares one | _should carry a lifetime and nothing else_ | D-146 |
| ~~and the group refuses one by name~~ — **F-132**: both `should refuse a unique slot from the unbounded position` rows passed by **weak-type detection**, which fires only for a literal sharing no member with an all-optional target, and F-74/CE1-01 records that as not a boundary. The falsifying control is now the row, in both suites | _should not refuse a unique slot at the group alone_ (both declaration suites) | F-132, F-74 |
| **the position is where the property lives** (D-151): an installer contributing a unique slot is refused at `sortable()`/`freeDrag()`, in the config and in a fragment, and the offender is named by slot even in a mixed array — which is what `const` type parameters buy | free drag's four — the sortable's have no subject left, since every sortable slot is a named key with one writer | D-151, F-145 |
| and the controls that keep it from becoming exact-shape validation: multi-writer slots still accumulate from many entries and across fragments, the same installer in its own key is untouched, and an explicitly widened one is accepted — the documented residual | _should accumulate several legitimate plugins_, _should accept an installer the bounds key legitimately takes_, _should accept a widened installer, which is the documented residual_ | D-151, F-74 |
| the unique-slot set is **derived** from the groups, and the precondition it rests on — no two non-plugin groups declaring one key — is asserted rather than assumed | _should derive the unique slots from the groups themselves_, _should hold the precondition the positional model rests on_ | D-151 |
| the two landing installers, whose return type is now literally one declaration, still refuse each other — the crossing D-146 created and no suite asserted | `tests/composition.declaration.test.ts` — _should still refuse the two landing installers to each other_, _should return one declaration from both landing installers_ | F-133, D-138 |
| and the controls beside them: a hook the sortable's plugin group does declare, and the lifetime free drag's does | _should accept the lifetime it does declare_ | F-74 |
| a `bounds` installer that contributes no constraint is **retired** rather than leaked — the guard that keeps free drag's dereference behind its own ledger entry, which is where its sibling's comment already said it should be | `tests/free-drag/validation.browser.test.ts` — _should retire a bounds installer that contributed no constraint_ | F-134 |
| a second key cannot drive a unique slot, asserted on an annotated `const` because a fresh literal in a return position is not excess-property-checked | _should refuse a constraint from the landing key_, _should refuse the axis geometry from the landing key_ | D-146, F-117 |
| both behaviors' `landing` key returns the **same declaration**, not two structurally equal ones | `tests/sortable/feature.declaration.test.ts` — _should return the same declaration free drag returns_ | B-7, F-64 |
| ~~two installers claiming a single-writer slot throw at construction, and the loser's contribution is retired first~~ — **three rows deleted 2026-08-27**: two in `tests/sortable/assemble.browser.test.ts` and one in `tests/free-drag/free-drag.browser.test.ts`. The unwind property they shared is still driven, by the only thrower left | `tests/sortable/assemble.browser.test.ts` — _should retire the hooks already collected when a factory throws_ | D-146, D-80 |
| a missing `axis` fails at the key's own call rather than three statements later at a null read, and still with no library message | `tests/sortable/options.node.test.ts` — _should not diagnose a missing axis with a library message_ | D-77, D-146 |
| the three installer aliases are **hoistable from the ordinary tier** on both behaviors, compiled out of line against the packed declarations | `tests/consumer.node.test.ts` — _should compile a consumer against the packed declarations_ | D-110, D-78, D-146 |

## Cancellation provenance — new (2026-08-29, D-154)

**One row per producer, and the mapping is the contract.** `origin` answers _who decided_, `stage` answers _when_, and `reason` answers _what the decider had to say_ and stays open. A producer routed to the wrong origin silently relabels a cause, and the field exists to be switched on — so the rows drive each producer the way a page does, at the kernel and again through the public entrypoint.

| Row | Test | ID |
| --- | --- | --- |
| `cancel(reason?)` is `CANCEL_SUPPLIED` whether the consumer or the behavior called it — the two are one function, so there is no second origin to assign | `tests/kernel/kernel.browser.test.ts` — _should mark a consumer cancel as supplied_, _should mark a behavior cancel as supplied_ | D-154 |
| Escape is `CANCEL_ABORTED`, and carries **no** reason — the kernel supplies no value for a cause it decided itself | ″ — _should mark Escape as aborted_, _should carry no reason for an Escape_ | D-154 |
| `pointercancel` and `lostpointercapture` are **one** origin, `CANCEL_INTERRUPTED` — two DOM spellings of one fact, and telling them apart would re-export the platform detail the kernel absorbs | ″ — _should mark a cancelled pointer as interrupted_, _should mark lost pointer capture as interrupted_ | D-154 |
| a classified failure is `CANCEL_FAILED`, with the caught throw still on `reason` — the value that pays for the field, since `canceled` with an `Error` on `reason` is otherwise indistinguishable from a consumer who passed one | `tests/sortable/composition.browser.test.ts`, `tests/free-drag/free-drag.browser.test.ts` — _should mark a classified failure as failed_ | D-154, D-66 |
| **and `reason` cannot forge it**: a consumer may pass a provenance constant to `cancel` — the channel accepts anything — and the result still reports `CANCEL_SUPPLIED`. This is the row that fails if provenance ever moves back onto the open channel | all three suites — _should not let a supplied reason forge an origin_ | D-154, D-143 |
| the behavior's own two reasons are **supplied values**, not provenance: they arrive on `reason` with `CANCEL_SUPPLIED`, and they are imported from `sortable.js` here, which is half of what the row checks | `tests/sortable/composition.browser.test.ts` — _should mark an invalidated collection as supplied_, _should mark a removed item as supplied_ | D-154, L-11 |
| the consumer's **own compiler** can name `CancelOrigin` and its values off each ordinary entry, discriminate on them, and is refused every use of `reason` that would look like one | `tests/consumer.node.test.ts` — the packed-consumer fixtures | D-154, F-59 |
| a destroyed controller publishes **no terminal at all**, so there is no fifth origin owed for it — F-172's correction, asserted rather than described | `tests/sortable/composition.browser.test.ts` — _should publish no terminal at all when the controller is destroyed_; `tests/free-drag/free-drag.browser.test.ts` — _should tear down an in-flight drag on destroy, with no terminal_ | F-172, I-31 |
| the four constants are on the frozen surface of all three behavior-facing entries, and on none of `drag.js` — the siting decision, asserted as an equality | `tests/exports.node.test.ts`, `tests/consumer.node.test.ts` | D-154, D-48 |
| **the interrupted pair is pinned in all three suites, not two** — the free-drag row was missing, and its absence let a split `case` routing `lostpointercapture` to `CANCEL_ABORTED` pass there while failing the other two (F-177, closed 2026-08-29). Run against the split: three files fail, where two did | `tests/free-drag/free-drag.browser.test.ts` — _should mark lost pointer capture as interrupted_ | F-177, D-154 |
| **a failure superseded by an already-latched cancel leaves the origin alone** — the terminal is the cancel's, the fault travels as a `DraggableWarning`, and `origin` stays `CANCEL_SUPPLIED` on an operation that also broke. `origin` names what _decided_, not everything that happened; `onError` is what answers _did anything go wrong_ (F-178) | `tests/sortable/composition.browser.test.ts` — _should apply work a callback queued before it threw_ | F-178, I-22, I-31 |

## What a committed move costs — new (2026-08-30, D-158)

The ownership rule made executable: **nothing that exists because something animates may live in the axis, and nothing may be constructed for a sink that is not there.** `tests/sortable/zero-read.browser.test.ts` carries the read counts and the allocation rows; the seam and release rows are next to the behavior they belong to.

**The reads instrument counts twice over, and the second count is the new one.** A total per committed move says how much layout work a composition does; it cannot say _when_. A read taken straight after a DOM write flushes layout, and the same read taken on the next animation frame finds a tree the browser has already laid out — so the fixture separates them, by queueing a microtask from the anchor row's own `before`/`after`. The committed move happens inside an animation-frame callback, so a microtask queued from it runs only once that callback has returned.

| Row | Test | ID |
| --- | --- | --- |
| a bare `xy()` forces **no** layout in the write's own task, and its total is unchanged — the rebuild moved to the following frame rather than disappearing | _should force no layout in the write's own task with a bare xy()_ | D-158, F-205 |
| the same axis **with a sink** measures where the write happened, at `COUNT` reads, because the vectors have to exist before the animations start. Without this row the one above reads as "`xy()` never forces layout" | _should measure inside the write's own task once xy() has a sink_ | D-158, F-205 |
| a settled `y()` forces none either, for the stronger reason that it reads nothing at all | _should force no layout in the write's own task with a settled y()_ | D-158 |
| **no buffer is built per committed move** in any of the four sortable compositions — the checkable half of "the visitor is passed in, so nothing is returned and nothing is allocated". One row per composition, named from the composition, under `describe('a committed move allocates nothing')` | D-158 |
| **release cancels nothing.** Driven against a landing composition, so the operation is still alive while the drop settles and teardown's own legitimate cancel cannot stand in for the deleted one | `tests/sortable/displacement.browser.test.ts` — _should cancel nothing when release resolves_ | D-158, F-204 |
| the release seam is **invalidate, then resolve** — two steps where there were three | `tests/sortable/sortable.browser.test.ts` — _should invalidate and then resolve at release, cancelling nothing_ | D-158, F-204 |
| **a throwing sink reports the stage the write reports**, because the visitor is called by the axis from inside the one wrapped hook and a stage says where the library was standing. The recovery is the part that matters and is unchanged: the cache may be half-advanced, so the bracket invalidates | `tests/sortable/sortable.browser.test.ts` — _should classify a throwing sink_ | D-158, D-159 |
| the seam is **one hook**, so one failure row replaces two: there is no second instant at which an axis can fail | `tests/sortable/sortable.browser.test.ts` — _should classify a throwing move hook_ | D-158 |
...
## The D-170 entity boundaries, instrumented — new (2026-09-04, F-316, F-317, F-318)

**Three guarantees the migration landed and left unfalsifiable.** Each is a property of a _type declaration_ or of a _closure's syntax_, so none of them can break loudly: a widened reader view, a member added beside `arm`, and an `async` wrapper all keep every existing row green while retracting what the entry claims. The three arcs that follow reapply the reader-view pattern, reshape the kernel's surface, and relocate the memoized destroy promise respectively — so these rows are the precondition for those arcs and not a tidy-up after them.

**Each row below was falsified by mutating the shipped source**, and the counts are in the entries. The pattern that matters is the same in all three: the mutation is invisible to the whole suite except at the row written for it.

| Row | Test | ID |
| --- | --- | --- |
| the reader view refuses all four writes the class performs on itself — `count`, a packed-buffer element, the hole's buffer, and the element array's length — plus an element assignment, which `readonly T[]` refuses separately from the length | `tests/sortable/rect-index.declaration.test.ts` — _should refuse a count assignment through the reader_, _should refuse a packed-buffer write through the reader_, _should refuse a hole write through the reader_, _should refuse an element-array mutation through the reader_, _should refuse an element assignment through the reader_ | F-316, D-170 |
| **the half a refusal set cannot supply**: the owner still writes all four through its own declarations, with no cast and through no second field. A view narrowed to nothing satisfies every refusal above and justifies deleting no accessor | _should let the owner write all four through its own declarations_ | F-316, D-170 |
| the view's member set, pinned in both directions, so a fifth member cannot arrive unnamed and the class's buffers still satisfy the reader's types | _should re-declare exactly the four data members_, _should hand the class's own buffers to the reader unchanged_ | F-316 |
| the behavior-facing narrowing as a **difference between two key sets**: `keyof BehaviorContext` is exactly seven, and `Exclude<keyof Kernel, keyof BehaviorContext>` is exactly `'arm'`. A member added to the class lands in the difference and a member promoted onto the interface leaves it, so neither can happen silently — which an assertion naming one member cannot say | `tests/kernel/context.declaration.test.ts` — _should grant exactly the seven members it names_, _should withhold every public member of the class it does not name_ | F-317, D-170 §The behavior-facing interface |
| `arm` is still on the class, so an interface narrowed to nothing cannot satisfy the two rows above. The direct read is not restated here: `void kernel.arm` under `@ts-expect-error` already runs against the **shipped** declarations, in §Kernel vocabulary's packed `BEHAVIOR` fixture | _should keep the arming handshake on the class_ | F-317 |
...
## The execution bracket, as its own entity — new (2026-09-04, D-180; repaired 2026-09-05, F-343…F-346)

**The latch, the drain, the admission boundary and deferred teardown moved out of `Kernel` and `ActionQueue` into one entity that states their invariants.** The rows that used to drive `drain` directly drive `ExecutionBracket` instead — the semantics are the bracket's now, so the rows are — and eight rows land beside them for properties nothing reached while the same facts were spread across two files. `clearQueue`'s row stays with the storage, which is all `queue.ts` still owns.

**Fifteen rows, and the standard every one of them meets is that some mutation reddens it and no other row.** That is §8's _each mutation reddens the row written for it and no other_, read as the per-row existence claim it is: a row earns its place by having a failure of its own. Other mutations moving two or three rows at once is a fact about the rows, not a breach of it — and a row for which **no** such mutation exists is not a row, which is why one migrated row was deleted rather than kept (below).

| Row | Test | ID |
| --- | --- | --- |
| the drain's order and pairing: FIFO over what a pass queued, each entry with its own argument, and a nested dispatch that appends and runs after the action that raised it rather than interrupting it | `tests/kernel/execution.node.test.ts` — _should process entries in FIFO order_, _should pair each entry with its own argument_, _should not interrupt the running action when dispatch nests_ | D-180, D-36 |
...
| a completed drain drops what it drained, so a later pass replays nothing | _should drop every drained entry so a later pass replays nothing_ | D-180 |
| **teardown's step 2 releases the arguments a pass left queued.** A resolver that dispatches and then closes leaves entries the post-admission drain never reaches, so the teardown's own release is the only thing that can drop them — asserted through a `WeakRef` and a real collection, because whether the queue still holds a value is not a question the entity's surface can answer | _should release the arguments a refused pass left queued_ | D-180, F-343 |
...
| the closure promise settles after the teardown callback has run, **and settles whether that callback returns or throws** — the one arm where the arc changed behaviour rather than relocating it, since the pre-arc shape left the promise pending forever when the last step threw | _should settle its promise only after the teardown callback has run_, _should settle its promise when the teardown callback throws_ | D-180, F-344 |
...
**The mutations, twenty-one of them, and what each reddens.** Each was applied to `src/kernel/execution.ts` and run against the file above; the table records every red, not the one the mutation was written for, and the **bold** ones are the fifteen that redden exactly one row — the witness that gives that row its own failure.

| Mutation | Rows red |
| --- | --- |
| the drained order reversed | 3 — FIFO, pairing, the mid-drain latch |
| only the action tags reversed | 2 — FIFO, the mid-drain latch |
| **the queue's tail reordered behind its first entry** | **1 — FIFO** |
| **every entry handed the first argument** | **1 — pairing** |
| **the re-entrant drain guard dropped** | **1 — the nested dispatch** |
| **a nested dispatch handled inline instead of appended** | **1 — the nested dispatch** |
...
| **the run latch cleared on the panic path instead of the drain's `finally`** | **1 — the replay row** |
| **the run latch cleared on the success path only** | **1 — the run latch** |
| **teardown's `clearQueue` dropped** | **1 — the queued-argument release** |
| `close()` tears down unconditionally | 4 — both deferral rows, both settle rows |
| `runIngress` does not open a transaction | 3 — the ingress deferral, both settle rows |
| **`dispatch` does not open a transaction** | **1 — the outermost-boundary deferral** |
...
| **the settle moved out of the teardown's `finally`** | **1 — the settle on a throwing teardown** |

**One migrated row was deleted, and the reason is the standard rather than the count.** _Should reach an entry appended during the same drain_ asserted that the appended entry runs; _should not interrupt the running action when dispatch nests_ asserts that it runs **and** runs after the action that queued it, recording `'appended'` in the same order array. So every mutation that reddens the first reddens the second, and no mutation can redden the first alone — the subsumption is by construction, not an accident of which mutations were tried. Keeping it while the seventh planned falsifier was dropped for being "an assertion with no failure of its own" was one standard applied twice with two answers.

**The latch's per-iteration read still needs no row of its own beyond the mid-drain one**, and that remains a result: hoisting the read reddens that row and nothing else.
...
| `#spec` names the behavior only once the pair it owns is composed, so a `destroy()` raised from inside a frame factory never reaches an uncomposed one — `arm()`'s unwind resets the frame that does exist | `tests/kernel/kernel.browser.test.ts` — _should reset only the composed frame when a factory destroys_ | D-181 |
...
| **`#spec` published before the pair is composed** | **1 — reset only the composed frame** |
| `arm()`'s unwind reads the field instead of its locals | 3 — the partial-pair scrub and two arm-failure rows |

**Nine rows pin the construction window rather than the entity**, in [`tests/kernel/construction.node.test.ts`](kernel/construction.node.test.ts) — six added on 2026-09-07 by D-184 and three on 2026-09-08 by Arc B's remediation. They are listed apart because the subject is different — `draggable()`'s two-phase handshake, not a frame, a seam or a queue — and because 1360 rows observed none of the behaviour they pin.

**The counts below are over the whole suite, and this table says so because the one above it does not count that way.** The D-181 table's right-hand column is scoped to the rows written for that entity; every figure here is over all 1364 rows the suite carried at `5a438352c`, in every project, and every entry that enumerates names all of them — it carries 1366 since Arc C added the two rows below. Each mutation was applied alone in a detached `git worktree`, never in the shared checkout. The three `consumer.node.test.ts` packed-package rows that an unpacked worktree fails by construction are red in that worktree's baseline too, and are excluded from every figure.

| Row | Test | Mutation, and what it reddens |
| --- | --- | --- |
| a spec that reached `arm()` is retired exactly once for any `destroy()`, including one raised from inside a frame-part factory | _should retire the behavior when a frame-part factory destroys the controller_ | the unwind reached only by `arm()`'s throwing exit — **5**: this row, _unwind both frames_, _compose no frame part_, _reset only the frame parts_, and the browser row _reset only the composed frame when a factory destroys_. The mutation is the whole property rather than one row of it |
| the terminal latch is what refuses to arm when both frames exist and the controller is closed | _should unwind both frames when the second frame-part factory destroys the controller_ | **the third latch test dropped — 5**: the same five rows, this being the only one on which the pair-narrowing that once stood beside the latch could not stand in for it |
| the frame reset covers exactly the frame parts that were composed | _should reset only the frame parts a destroying factory composed_ | **the unwind's `if (draft)` dropped — 4**: this row, _reset no frame part_, and the browser rows _reset only the composed frame when a factory destroys_ and _scrub the first frame when the second factory throws_ |
| a window that composed nothing is handed nothing to reset | _should reset no frame part when the factory body destroys the controller_ | **the unwind's `if (current)` dropped — 1**, this row alone. The two guards are separately deletable and are booked separately: `frame(null)` yields `{}` rather than throwing, so dropping either one hands `resetFramePart` a frame the behavior never produced |
| a `destroy()` from the factory body composes no frame part and still retires | _should compose no frame part and still retire when the factory body destroys the controller_ | **the latch test before the first composition dropped — 2**: this row and _reset no frame part_ |
| `fail()` from the construction window demotes instead of throwing | _should demote a fail() raised from the factory body instead of throwing_ | **the `#spec` guard on `fail` dropped — 1**, this row alone |
| `cancel()` from the construction window is the idle no-op | _should treat a cancel() raised from the factory body as the idle no-op_ | **the `#spec` conjunct in `#cancelWith` dropped — 1**, this row alone |
| the `actionTags` range is refused whatever the controller's liveness | _should refuse an invalid configuration even when the factory destroyed the controller_ | **the static validation gated on the terminal latch — 2**: this row and the `command.types` one below. Moving the validation _below_ the first latch test without gating it on the latch reddens **0**, which is the mutation the property is not about |
| the `command.types` collision is refused on the same terms | _should refuse a colliding command type even when the factory destroyed the controller_ | the same mutation, the same **2**. The two inputs are validated separately and both are pinned; no mutation in this register separates the two rows, and none is claimed to |

**Two rows in different layers pin the same reset and neither witness separates them** (F-396). The browser row _should reset only the composed frame when a factory destroys_ has the trigger of the node row _should reset only the frame parts a destroying factory composed_ and a strictly weaker assertion — `resets === 1` against `[creates, resets] === [1, 1]` — and every mutation above reddens both or neither. It is kept for its layer rather than for a witness of its own: it drives a real root, a real `addEventListener` and a real `destroy()`, which the node file's stub root does not. **The D-181 table's entry for it stays scoped to that table** and is not a claim that the two rows discriminate.
...
| `current.operation === pinned` dropped | **none, and none could be written over any in-contract path** — see the reasoning below, repaired. The conjunct is deleted since 2026-09-07 (D-186) |

**The third conjunct has no falsifier reachable through correct use of the contract, so `#pinned` did not move at the arc** — and the reason recorded here was wrong in one clause, repaired 2026-09-07 (D-186), which then deleted the conjunct and the field together. **The claim is an in-contract one and is written as one**: what the register records is that no path correct use reaches can falsify it. Three clauses hold: `#retireOperation` nulls `current.operation` and clears `#pinned` with nothing between them, `cancel()` is caught by the second conjunct and `destroy()` by the first. ~~The only other writer of `current.operation` is an admission the execution bracket refuses reentrantly.~~ **Retired: [`execution.ts`](../src/kernel/execution.ts)'s `runIngress` refuses on `#closed` and `#admitting`, which spans an admission and not a seam phase.**

**What excludes the path is two stated boundaries, and they are stronger than the clause they replace.** `#openIngress` refuses whenever an operation is live, which closes every `runCore` caller but `#handleBehaviorAction`, the one seam that can run at `IDLE`. And reaching a nested ingress from inside a `prepare` needs a **synchronous** dispatch on `root`, which every platform mechanism produces only as a DOM write — `focus()`, `click()`, `dispatchEvent()`, removing the focused node — while [`02`](../.plan/contract/02-kernel-behavior-contract.md) §The tri-phase transition forbids a DOM write from `prepare`. What `prepare` may do dispatches nothing: layout reads fire no events, the three observers deliver asynchronously, and a real gesture cannot interleave with synchronous JavaScript.

**Executed at both trees, from all three candidate positions.** From `action.effect` and from inside `onStart` — the contract-legal ones — the tree with the conjunct and the tree without it are indistinguishable. Only a DOM write from `prepare` separates them, and that is a behavior breaking a documented rule. **Both shipped behaviors close it twice over**: free drag declares no `command` member and discards every action outside `ACTIVATING` and `ACTIVE`, and the sortable's only command type is `keydown`, which no DOM write dispatches synchronously.

**So the mutation was equivalent** over every path correct use of the contract reaches, which is what this register records. Relocating a guard into a new entity in the same commit that would justify deleting it makes the deletion unreviewable afterwards, so the field stayed where it was and the arc recorded the finding instead. **D-186 performed the reading on 2026-09-07 and deleted the conjunct, `#pinned` and the driver's fifth collaborator**; the entry is kept here as the record of the mutation that decided it, not as a live guard.
...
## I-4's double validation, instrumented — new (2026-09-08, F-400)

**Two rows, one per half, at the kernel harness.** I-4 validates a resolution completion twice — once where it settles and once where the queued action is applied — and until this pass **neither** comparison had a falsifier: deleting `#attempts.resolution !== attempt` from `#settleResolution`, from `#handleResolutionSettled`, and from both at once each left the suite green. The sortable's rows pass either way, because a behavior's own staleness refusal answers before the kernel's does; these are driven through [`kernel.browser.test.ts`](kernel/kernel.browser.test.ts)'s harness, which has no behavior of its own to answer first.

**Each half needs a window the other cannot see into**, which is why neither row is the obvious _a stale resolution settles late_. That shape is refused by whichever guard runs first and reddens nothing when only one is deleted. A settlement whose `prepare` **throws** opens both windows, and it is the shape the seam documents for a behavior that finds no coherent settlement: `#openSettlement` clears the slot on its way in, and a failed preparation commits no phase and never reaches the effect that releases the operation's lifetimes.

| Row | Test | Mutation, and what it reddens |
| --- | --- | --- |
| a completion for an attempt the slot no longer holds is refused where it settles, so the attempt never latches `completed` and the abort it owes the resolver still fires | _should refuse a completion whose slot turned over before it settled_ | **`#settleResolution`'s `#attempts.resolution !== attempt` dropped — 1**, this row alone. The completion is raised from inside `settlement.prepare`, where the slot is already empty and the cancellation lifetime is still open; with the conjunct gone the stale attempt latches `completed`, and `useWhile`'s guard then suppresses the abort the failure report's own effect would otherwise have fired |
| a completion that owned the slot when it settled is refused when it is applied, once a settlement has turned the slot over with the phase still `RELEASING` | _should refuse a completion whose slot turned over before it was applied_ | **`#handleResolutionSettled`'s `#attempts.resolution !== attempt` dropped — 1**, this row alone. `cancel()` from inside `invoke` queues `CANCEL` ahead of `RESOLUTION_SETTLED`; the cancel's settlement clears the slot and then fails to prepare, so the phase check sees `RELEASING` and passes the stale completion through to a settlement between the cancel's and the failure report's |

**The counts are over the whole suite and over a stated population**: all 1,366 rows in every project, at a detached worktree of `06ad3ff87` carrying this arc's source and tests, with each mutation applied alone and never in the shared checkout. The three `consumer.node.test.ts` packed-package rows that an unpacked worktree fails by construction are red in that worktree's baseline too, and are excluded from every figure. **One flake is reported rather than absorbed**: `M-5 arm A` — _should report a difference that tracks a cost injected into window 1_ — failed once under the first mutation and passed on the re-run that produced its figure, on a machine running builds beside it. It is a timing instrument and is not attributed to the mutation.

**Two neighbouring conjuncts were run for the same population, because _reddened by its own conjunct and by no other_ is a claim about them too.** Dropping `attempt.completed` from `#settleResolution` reddens **1** — _should keep the first completion when a thenable resolves and then throws_ — and dropping `phase !== RELEASING` from `#handleResolutionSettled` reddens **0**, which is [§Equivalent mutants](#equivalent-mutants--guards-no-test-can-falsify)'s entry rather than a gap in these two rows.