...
| ~~a frame part declaring `phase` is rejected in production~~ / ~~a symbol-keyed frame part is rejected~~ — **`validateFramePart` is deleted** (D-124, then D-122 with its last arm). Composition accepts every part a factory returns; the terms are published on `FramePartOf`, and the rows below are acceptance tripwires only — what each misuse then does is in D-121 … D-124, not in an assertion | `tests/kernel/frames.node.test.ts` — _should accept a symbol-keyed part_, _should accept a part declaring a kernel frame key_, _should accept an own prototype-mutating data property_, _should accept an accessor part_, _should accept a non-enumerable key_, _should accept a non-writable key_, _should accept a class instance_ | F-20, D-122, D-124 |
