...
| **a throwing sink reports the stage the write reports**, because the visitor is called by the axis from inside the one wrapped hook and a stage says where the library was standing. The recovery is the part that matters and is unchanged: the cache may be half-advanced, so the bracket invalidates | `tests/sortable/sortable.browser.test.ts` — _should clear the gap when the sink throws_ | D-158, D-159 |
| the seam is **one hook**, so one failure row replaces two: there is no second instant at which an axis can fail | `tests/sortable/sortable.browser.test.ts` — _should clear the gap when the move hook fails_, _should invalidate when the move hook fails and not when it succeeds_ | D-158 |
