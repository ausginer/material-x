...
**Six rows were added on 2026-09-07 by D-184, over the construction window rather than over the entity**, in [`tests/kernel/construction.node.test.ts`](kernel/construction.node.test.ts). They are listed apart because the subject is different — `draggable()`'s two-phase handshake, not a frame, a seam or a queue — and because 1360 rows observed none of the behaviour they pin.

| Row | Test | Mutation, and what it reddens |
| --- | --- | --- |
| a spec that reached `arm()` is retired exactly once for any `destroy()`, including one raised from inside a frame-part factory | _should retire the behavior when a frame-part factory destroys the controller_ | the unwind reached only by `arm()`'s throwing exit — **3**, this row and the two below it, because the mutation is the whole property rather than one row of it |
| the frame reset covers exactly the frame parts that were composed | _should reset only the frame parts a destroying factory composed_ | **the unwind resetting both frames unconditionally — 1** |
| a `destroy()` from the factory body composes no frame part and still retires | _should compose no frame part and still retire when the factory body destroys the controller_ | **the latch test before the first composition dropped — 1** |
| `fail()` from the construction window demotes instead of throwing | _should demote a fail() raised from the factory body instead of throwing_ | **the `#spec` guard on `fail` dropped — 1** |
| `cancel()` from the construction window is the idle no-op | _should treat a cancel() raised from the factory body as the idle no-op_ | **the `#spec` conjunct in `#cancelWith` dropped — 1** |
| a refused configuration throws its `TypeError` even when the factory destroyed the controller first | _should refuse an invalid configuration even when the factory destroyed the controller_ | **the terminal latch tested ahead of the static validation — 1**. Moving the validation _below_ the first latch test without gating it on the latch reddens **0**, which is the mutation the property is not about |
