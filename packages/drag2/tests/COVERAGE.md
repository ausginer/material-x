...
**The counts below are over the whole suite, and this table says so because the one above it does not count that way.** The D-181 table's right-hand column is scoped to the rows written for that entity; every figure here is over all 1364 rows in every project, and every entry that enumerates names all of them. Each mutation was applied alone in a detached `git worktree`, never in the shared checkout. The three `consumer.node.test.ts` packed-package rows that an unpacked worktree fails by construction are red in that worktree's baseline too, and are excluded from every figure.
