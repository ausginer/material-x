...
| the geometry's coordinates are scalars, and the request's with them | `tests/consumer.node.test.ts` — the `@ts-expect-error` on `request.viewportDelta`; the free-drag geometry and activation-snapshot suites, re-pointed field by field | D-139 |
