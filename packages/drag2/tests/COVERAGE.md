...
| no identifier is claimed twice across the current-state tree, and the report names both sites — **including twice inside one document**, which is the shape a canonical-id collision in the findings register takes, since the register is one file | `tests/decisions.node.test.ts` — _should report one identifier claimed by two documents_, _should report one identifier claimed twice in the same document_, _should hold the invariant across every current-state document_, _should read enough of the tree for that to mean something_ | D-174, D-193, F-287 |
