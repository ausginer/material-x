...
| no identifier is claimed twice across the current-state tree, and the report names both sites | `tests/decisions.node.test.ts` — _should report one identifier claimed by two documents_, _should hold the invariant across every current-state document_, _should read enough of the tree for that to mean something_ | D-174, F-287 |
