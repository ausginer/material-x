...
| a frame part declaring `phase` overwrites the kernel slice, and is **not** rejected in production — the type makes it uninhabitable instead (D-124) | `tests/kernel/frames.node.test.ts` — _should accept a kernel frame key, which overwrites the kernel slice_ | F-20, D-124 |
...
| ~~the retained `=== Infinity` guard refuses **both** input forms at settlement~~ — **deleted D-124**; both input forms now hold the settlement gate open, and both are still pinned | `tests/sortable/features.browser.test.ts` — _should hold the settlement gate for an unbounded fixed duration_, _should hold the settlement gate for an unbounded contextual duration_ | D-77, P18A-19, D-124 |
| the cost of the boundary is the terminal: an unbounded duration publishes **none at all** | `tests/sortable/features.browser.test.ts` — _should publish no terminal at all for an unbounded duration_ | D-66, D-77, P18A-19, D-124 |
...
| ~~a landing `duration` of `Infinity` is refused~~ — **deleted D-124**; it holds the settlement gate open and publishes no terminal | `tests/free-drag/validation.browser.test.ts` — _should hold the settlement gate open for an unbounded duration_ | B-4 (d), D-24, D-79, E-07, D-124 |
...
| **the coupling, executed rather than traced** (D-124): a non-finite `moveTo()` reaches the library-minted `LandingTimingContext.distance` a conforming `duration` thunk is called with — `Infinity` compounds into a held settlement gate with no terminal, `NaN` stays classified because `animate()` refuses it | `tests/free-drag/validation.browser.test.ts` — _should mint a non-finite distance for a conforming duration thunk_, _should compound into a held settlement gate for an unbounded distance_, _should stay classified for a NaN distance, which the platform refuses_ | D-124 |
