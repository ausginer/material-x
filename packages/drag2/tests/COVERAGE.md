...
| a constraint whose members are **lifted out of the record** works at every site, retires at both, and the first-party `bounds()` is the recorded non-discriminating control | _should be called without this at every site_, _should retire detached as well_, _should leave the first-party bounds() working through the same sites_ | D-90, CE1-03, F-74 |
