...
| cross-behavior installers do not compile in **either** direction, `AxisInstaller → FreeDragPlugin` named explicitly | `tests/composition.declaration.test.ts` — _should not be assignable to a free-drag installer_, _should not be assignable to a free-drag plugin_, _should not be assignable to a sortable plugin_, _should not be assignable to an axis installer_ | E-06, D-87, D-138 |
...
| Row | Test | ID |
| --- | --- | --- |
| each of the four lifted members is driven **alone** and is never handed the nested geometry it is declared on, so re-binding a single lift in `assemble` fails a single row | _should hand the resolve site a foreign receiver_, _should hand the invalidate site a foreign receiver_, _should hand the optional measure site a foreign receiver_, _should hand the retire site a foreign receiver_ | D-92 |
...
| the unbounded position reaches **no** unique slot in either behavior, because neither plugin group declares one | _should carry multi-writer slots only_, _should carry a lifetime and nothing else_ | D-146 |
...
| **the position is where the property lives** (D-151): an installer contributing a unique slot is refused at `sortable()`/`freeDrag()`, in the config and in a fragment, and the offender is named by slot even in a mixed array — which is what `const` type parameters buy | _should refuse an axis installer from the plugins position_, _should refuse a landing installer from the plugins position_, _should refuse one offender among legitimate plugins_, _should refuse an offender in a fragment_, and free drag's four | D-151, F-145 |
| and the controls that keep it from becoming exact-shape validation: multi-writer slots still accumulate from many entries and across fragments, the same installer in its own key is untouched, and an explicitly widened one is accepted — the documented residual | _should accumulate several legitimate plugins_, _should accept an installer the axis key legitimately takes_, _should accept an installer the bounds key legitimately takes_, _should accept a widened installer, which is the documented residual_ | D-151, F-74 |
...
| and the controls beside them: a hook the sortable's plugin group does declare, and the lifetime free drag's does | _should accept a hook it does declare_, _should accept the lifetime it does declare_ | F-74 |
