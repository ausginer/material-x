...
| **the entry barrier**: a `refresh` entered already closed calls the resolver **zero** times — the release path, where the invalidation runs immediately before `resolveInsertion` | `tests/sortable/{y,xy}.browser.test.ts` — _should call no resolver at all when the controller is already closed_ | I-36 |
