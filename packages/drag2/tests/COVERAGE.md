...
| the structural limits, typed: `ActivationScope.lift` and `moved`'s argument expose neither `renderedX`/`renderedY` nor `dispose`, and the kernel's own session keeps both | `tests/kernel/spec.declaration.test.ts` — _should not expose the recorded delta on the activation scope_, _should not expose the disposer on the activation scope_, _should hand `moved` the same projection and not the session_, _should keep both members on the kernel's own session_ | I-34, C5-01 |
