...
| ~~two installers claiming the motion constraint is the one construction-time throw~~ — **deleted 2026-08-27** (D-146). The throw is gone with the discovery it arbitrated: `constrain` is declared on the `bounds` key's group and nowhere else, so the composition the row constructed no longer compiles. The property moved one tier up, to the type-surface row below | `tests/free-drag/feature.declaration.test.ts` — _should refuse a unique slot from the unbounded position_ | ~~D-77~~ D-146 |
...
| the unbounded position reaches **no** unique slot in either behavior, because neither plugin group declares one | _should refuse a unique slot from the unbounded position_ (both declaration suites), _should carry multi-writer slots only_, _should carry a lifetime and nothing else_ | D-146 |
