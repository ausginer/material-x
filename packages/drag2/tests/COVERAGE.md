...
| **a throwing sink is an invalidation failure**, because the visitor is called by the axis from inside the one wrapped hook. The recovery is the part that matters and is unchanged: the cache may be half-advanced, so the bracket invalidates | `tests/sortable/sortable.browser.test.ts` — _should clear the gap when the sink throws_ | D-158 |
