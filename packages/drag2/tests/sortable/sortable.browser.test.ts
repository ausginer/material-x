...
type Harness = Readonly<{
  ...
  errors: Array<Readonly<{ stage: FailureStage | null; error: unknown }>>;
 ...
 * `errors` projects to a `stage` and half this population has none.
 */
...
function createHarness(overrides: Overrides = {}): Harness {
  ...
  const errors: Array<
    Readonly<{ stage: FailureStage | null; error: unknown }>
  > = [];
...
describe('the admission queue boundary', () => {
  ...
  it('should classify a throwing visual resolver as a consumer fault', async () => {
    ...
    //
    // The classification was already right; what was missing was the
    // instrument. ~~The coarse code is read from the kernel's own mapping
    // rather than retyped, so a remap fails here instead of agreeing with a
    // stale literal.~~ **D-132 removed the mapping and the indirection with
    ...
    expect(harness.errors.map((entry) => entry.stage)).toEqual([
      FAILURE_ADMISSION,
    ]);
...
describe('the hot path', () => {
  ...
  it('should displace once, after the placeholder write', async () => {
    ...
    expect(seen).toEqual(['displace']);
    expect(harness.calls).toEqual(['resolveInsertion']);
  });
});
...
describe('the placeholder container guard', () => {
  ...
  it('should refuse a spatial move whose anchor left the container', () => {
    ...
    return nextFrame().then(() => {
      expect(harness.errors).toHaveLength(1);
      expect(harness.errors[0]!.stage).toBe(FAILURE_ACTION_EFFECT);
  ...
  it('should refuse a home recovery whose anchor left the container and still cancel', () => {
    ...
    // warning and carries no stage at all.
    expect(reported).toHaveLength(1);
    expect(reported[0]).toBeInstanceOf(DraggableWarning);
...
describe('the displacement view lifetime', () => {
  ...
  it('should clear the gap when the lazy invalidation fails', async () => {
    ...
    // The second failure really was raised, and the terminal keeps the first:
