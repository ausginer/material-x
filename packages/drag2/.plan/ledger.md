...
| `AnimationTiming` = `Pick<EffectTiming, 'duration' \| 'easing'>` | **drop** | Dissolved into `LandingOptions`/`LayoutAnimationOptions` and **not itself public** — decided at Checkpoint D, loss stated in the §2.1 row (a real shipped named export, and the annotation consumers used for `landingTiming()`). Classified _redesign_ until review 2; a name that no longer exists on the surface is a **drop** whatever replaces its members, which is the same reading §2.1's `DragSubject` row already takes. Phase 15's `duration` domain is *different*, not wider — see §2.1. |
...
- **L-6 — settle-time `landingTiming()`. ~~The one capability drag2 cannot express.~~ Corrected by probe 13b: it fits. Closed at Phase 15; the finding text below was stale until Checkpoint D review 2 (C2-02).**
  ...
  *What probe 13b established:* `landing({ run })` already accepts a full replacement `LandingStart`, and the kernel invokes a runner during _arming_ — after `settlement.effect` returns and after `anchorTarget` (`src/kernel/kernel.ts:1208`), exactly the moment the shipped package read `landingTiming()`. So the capability was reachable even before Phase 15; only the **ergonomics** were short of parity, because a consumer wanting a distance-scaled duration had to reimplement the default runner and lose its reduced-motion collapse, retarget replay and generation guard. See [`probes/13b-settlement.md`](probes/13b-settlement.md) §B-2.

  *What closed it:* **Phase 15 shipped the ergonomic form.** `LandingOptions.duration` accepts `number | (() => number)`; the thunk is invoked and validated once per landing, ahead of the reduced-motion collapse, and the default runner is kept. This row called that "a public-option change for Phase 15 or 22" long after Phase 15 made it, which is what C2-02 found; there is **no future phase owed** here. What remains open on this axis is only whether `easing` should take a thunk too, and nothing asks for it.
