...
- **L-6 — settle-time `landingTiming()`. ~~The one capability drag2 cannot express.~~ Corrected by probe 13b: it fits. Closed at Phase 15; the finding text below was stale until Checkpoint D review 2 (C2-02).**
  ...
  _What closed it:_ **Phase 15 shipped the ergonomic form.** `LandingOptions.duration` accepts `number | (() => number)`; the thunk is invoked and validated once per landing, ahead of the reduced-motion collapse, and the default runner is kept. This row called that "a public-option change for Phase 15 or 22" long after Phase 15 made it, which is what C2-02 found; there is **no future phase owed** here. What remains open on this axis is only whether `easing` should take a thunk too, and nothing asks for it.
...
The nine drops are seven capabilities, each with its loss stated: `SortableResult.is*`, `FreeDropResult.is*`, `FreeHomeRequest`, `MaybePromise`, `ResolutionContext`, `DragSubject` (two rows, §2.1 and §7) and `AnimationTiming` (two rows, §2.1 and §7).
