...
**Disposition:** **Discharged 2026-09-09 by Arc D's landing**, which is the close the row waited on, and by the one act that close authorises. The five rows carry exact `control:` figures again — **8,116**, **8,274**, **8,276**, **8,426** and **6,185** — and the suspension paragraph is gone from the `control?` docblock, which is what the deferred row witnessed. **The equalities returned to a tree they had not moved in**: all five read byte-identical across the arc on both the minified and the Brotli column, with no module gained, so the restoration re-declares figures the pass did not produce ([`arc-d.md`](measurements/arc-d.md) §The controls, and O-13). **They were checked for inertness rather than assumed live**: declaring `kernel root - kernel.js` at 6,186 reports `control moved by −1 B`. **What comes back is exactly what was withdrawn** — detection of a byte _transfer_ between rows, which every ceiling can stay green through — and nothing else was ever withdrawn, the `budget:` and the graph declarations having stayed in force throughout. **The close spent nothing** ([`CONTRIBUTING.md`](../../../CONTRIBUTING.md) §18, D-187): the ceilings those five rows keep are untouched, and the re-base §18 might have asked of the eight rows that _did_ move was declined at the reading, not at the boundary ([`arc-d.md`](measurements/arc-d.md) §No budget re-base). **The count in force was four** — Arcs A (D-180), B (D-181), C (D-189) and D (D-191) — and the register agreed with itself about that before the boundary was tested, which is what F-405 was raised to secure
