...
**(b) A destination must be a thing that can close, and closing it discharges or re-books every obligation booked to it.** This is a rule rather than an instrument because **no instrument can observe a destination closing**. `tests/decisions.node.test.ts` cannot spell `Phase R`, `Checkpoint E`, _a measurement_ or _the owner_ — its destination vocabulary is `Phase <n>`, `Before Phase <n>` and `Remediation` — and its witnesses are source-level facts, which cannot see a phase end. What _can_ observe a close is the pass that performs it. Phase R, Checkpoint E and the API deliverable each closed correctly on their own terms and each left something behind, because nothing asked them at the moment of closing. **Asking is now part of closing** — **and part of abandoning** (D-187). A destination given up rather than reached re-books what was booked to it, in the pass that gives it up; a live row pointing at a destination that will never arrive reads as _still pending_ rather than as orphaned, which is API-03's shape with the clause moved outside the row. What neither half survives is a destination nobody ever decides about, and that is why a boundary is a legitimate trigger only where its abandonment is itself an act ([`CONTRIBUTING.md`](../../../CONTRIBUTING.md) §18).
...
**Owed by:** F-385, adjudicated with D-186

**Waiting for:** **The close of the D-170 arc series** — Arc D, the last of the four. **Re-checked 2026-09-09 (D-190, re-confirmed by D-191) and confirmed**: the four are Arcs A, B, C and D, D-189 having run a pass under the letter C — a deletion, two falsifiers and [`arc-c.md`](measurements/arc-c.md) — after D-188 withdrew that arc's entity **extraction**. D-189's own disagreeing count is corrected where it stands (F-405), not here, because this row's number was the right one. A control is _omitted on a row a pass is expected to move_ by the instrument's own rule, and every kernel-tier pass is expected to move all five; re-declaring them after each arc produces a number read off the result, which is the failure [`budget-rebases.md`](measurements/budget-rebases.md) names as _a budget wearing an exact number_. They are therefore omitted for the series rather than re-based through it, and restored with fresh exact figures when the last arc lands.

**The boundary is a performed act, which is what makes it a trigger rather than a schedule** ([`CONTRIBUTING.md`](../../../CONTRIBUTING.md) §18, D-187). The series ends because a pass closes Arc D, and it is given up only because a decision gives it up, so neither outcome arrives with nobody standing at it. **The close restores the five controls and authorises nothing else** — in particular it is **not** a re-base trigger for the ceilings those rows keep, which stay administered by §18 like every other row and move on a reading rather than on a boundary. ~~What the close supplies is the next occasion on which those ceilings are read~~ — **corrected 2026-09-08 (D-189): the close supplies _an_ occasion and not the next one.** Every pass inside the series reads these ceilings too, because a suspended `control:` withdraws an equality assertion and leaves the `budget:` in place, and Arc C's measurement is one such reading ([`arc-c.md`](measurements/arc-c.md)). §18 decides at each of them whether the ceilings move — a reading well under budget ends in a re-base and one over it ends in a decision — so a re-base may land before the close, at the reading that earns it, and the close is left with the one act it can perform: returning the five controls.

**What the suspension withdraws, stated as the capability rather than as the coverage that remains** (D-187): on these five rows nothing now detects a **byte transfer** — bytes moving between rows while every ceiling stays green, which is the one failure a control exists to catch and a budget structurally cannot. Nothing else is withdrawn. Each row keeps its `budget:` and its graph declarations, and those carry their own claims unchanged: `absent`, `absentPrefixes` and `present` on the four free-drag rows, `present` and `absentPrefixes` on `kernel root - kernel.js`. **This row is the whole of the protection**: with the five omitted, nothing else in the tree would notice that they never came back. **What the restoring pass should expect to read** (D-190, narrowed by D-191): Arc D edits `src/sortable/` alone, so these five rows are predicted **byte-identical** across it, and the restoration re-declares figures that arc did not move — which is the condition a returning equality wants. **The prediction is entailed rather than independent**, which is what it is worth: each of the five already declares `absentPrefixes: ['sortable/']`, so no `sortable/` module is in any of their graphs and identical module sets compress identically. A non-zero delta is therefore not a tier-crossing report but an edit outside `src/sortable/` or a non-deterministic build, and either answers itself; the restoration waits on it being answered all the same. **Omitted 2026-09-07 with Arc B's remediation**, whose measurement records their last reading before the suspension ([`arc-b-remediation.md`](measurements/arc-b-remediation.md) §The controls) so the restoration has a figure to be read against rather than only a fresh one
