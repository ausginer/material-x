...
**§18's boundary test has two limbs and this rule points at both** (D-192, F-410). The abandonment limb is necessary and is not sufficient: a boundary can also stop being **identifiable** without anyone abandoning it, and then it arrives with nobody deciding anything — which is what happened to this file's own O-13 while the register held both _three arcs_ and _Arc D, the last of the four_ (F-405). So a boundary is a legitimate trigger only where **one identification of it is stated where the obligation is administered**, so an observer standing at a candidate close can decide whether this is it. Both limbs are asked of every row that names a boundary; passing the first alone is what the counterexample passed.
...
**Disposition:** **Discharged 2026-09-09 by Arc D's landing**, which is the close the row waited on, and by the one act that close authorises. The five rows carry exact `control:` figures again — **8,116**, **8,274**, **8,276**, **8,426** and **6,185** — and the suspension paragraph is gone from the `control?` docblock, which is what the deferred row witnessed. **The restoration was read against the figures the suspension recorded** (§18, D-192, F-409): **8,136**, **8,298**, **8,295**, **8,447** and **6,210**, giving a suspension interval of **−20, −24, −19, −21 and −25 B**, every byte of it attributable to F-394's deletion and to Arc C's, with the chain continuous at each seam ([`arc-d.md`](measurements/arc-d.md) §The controls). ~~The equalities returned to a tree they had not moved in … so the restoration re-declares figures the pass did not produce~~ — **struck**: byte-identity **across the arc** is a scope tripwire and is neither necessary nor sufficient for a returning equality, and it is what stood here in place of the read. **They were checked for inertness rather than assumed live**: declaring `kernel root - kernel.js` at 6,186 reports `control moved by −1 B`. **What comes back is exactly what was withdrawn, stated against what the instrument asserts rather than against the `control?` docblock's motivating case** (§18, D-192, F-411). `controlViolations()` compares `brotli !== composition.control` and reports **either** direction, so what the five rows detect is **any** Brotli movement at all on a row declared unreachable — a byte _transfer_ between rows is the motivating case, and a control getting _cheaper_ is as much a finding, because it means a change reached a graph it was declared unable to reach. The narrower reading understated the loss, and the suspension is what it cost: one commit into it, `free drag + landing` moved **+11 B** and `free drag complete` **+12 B** on a pass whose production delta was a −6 B minified deletion ([`arc-b-remediation.md`](measurements/arc-b-remediation.md), second table). Under a live control those are two reported findings; the record explains them in prose. The `budget:` and the graph declarations stayed in force throughout, and this row asserts nothing about what they cover. **The close spent nothing** ([`CONTRIBUTING.md`](../../../CONTRIBUTING.md) §18, D-187): the ceilings those five rows keep are untouched, and the re-base §18 might have asked of the eight rows that _did_ move was declined at the reading, not at the boundary ([`arc-d.md`](measurements/arc-d.md) §No budget re-base). **The count in force was four** — Arcs A (D-180), B (D-181), C (D-189) and D (D-191) — and the register agreed with itself about that before the boundary was tested, which is what F-405 was raised to secure
