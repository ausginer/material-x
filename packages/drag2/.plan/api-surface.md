...
- ~~**`FailureStage`'s doc claims a consumer role D-64 removed.**~~ **Closed 2026-08-26 (D-132).** `DraggableError.stage` is a `FailureStage | null`, and `drag.js` publishes the type together with the twelve `FAILURE_*` constants as runtime exports — one declaration in `kernel/failures.ts`, two publication points. The sentence in that file is restored unstruck rather than repaired, because nothing about it was ever wrong except its premise; **the type is correctly kernel-tier-_and_-shared now**, and the divergence this bullet recorded no longer exists in either direction.
...
**The cost was measured rather than estimated, and it exceeded the slack as predicted.** D-108 is **282–305 B** per composition and **no module appeared** — the whole of it is text that used to fold away. Twelve budgets re-based in `bench/size/measure.ts` with the reason beside the numbers, under M-3′'s standing rule. `baseline B` did not move because it is the shipped predecessor; the `drag.js` vocabulary root is byte-identical at **121 B**, which is F-77's assertion confirming the error vocabulary still does not pull the kernel on a change that touched the kernel.

**~~283–340 B~~ was published first and was wrong on seven rows** (API-01, corrected 2026-08-22). It subtracted the _Landed figures_ list of the previous re-base rather than the pre-slice tree — and D-103 and D-104 had moved seven sortable rows after that list was written, so **14–46 B of P-06 and P-02 cost was attributed to D-108** and the quoted upper bound of 340 B matched no row. **The budgets are unaffected and unchanged**: each is the true landed figure plus ~150 B, and all fourteen are green. The rows whose first figures were already right are exactly the ones D-103 and D-104 never touched — the four free-drag compositions, `kernel root`, `baseline B` and `drag.js`.

**It is the fifth instance this phase of the class this document closes on by name** — _a proxy read where the quantity was meant_ — and it was taken inside the pass that names the pattern, using a list a previous review had already flagged as going stale. The remedy in `bench/size/measure.ts` is to record the **pre-change** measurement beside the landed one, so the next re-base has the subtrahend instead of a list it has to trust.
