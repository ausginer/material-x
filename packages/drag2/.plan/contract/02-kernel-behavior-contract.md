...
| **B** | **A phase never opens while a phase is open** | Two enforcement points, at different levels: `drain` returns while `queue.running`, so every behavior- and consumer-facing entry appends instead of interrupting; and `refuseReentry` refuses at the driver, ahead of `begin()` |
