...
**Construction-time composition of a known, closed set of sortable seams.** It is not an open plugin architecture, and describing it as one would be a straightforward overclaim: every new semantic seam requires coordinated edits to the public config schema, the contribution group its key carries, `SortableSlots`, `assemble`, validation, the behavior's call sites, the exports and the tests. Features cannot contribute controller methods, and cannot contribute transactional frame state.
...
type FeatureContext = Readonly<{
   ...
   * classified by the kernel's driver at that seam's stage.
   */
...
type SortableConfig = Readonly<{
  ...
  onStart?: SortableOnStart; // D-109, qualified per behavior
  onEnd?: SortableOnEnd; // D-62, one terminal, four arms
  onError?: SortableOnDragError; // D-64
...
**A public type's closure resolves within its own tier plus the tiers below it** (D-78). The ordinary tier closes over `sortable.js ∪ drag.js ∪ sortable/feature.js`; the kernel tier over `kernel.js ∪ drag.js` (D-68). `AxisInstaller`, **`SortableLandingInstaller` and `SortablePlugin`** are re-exported from `sortable.js`, because `SortableConfig` names them and a consumer must be able to hoist an installer into a typed `const` — the `landing?` and `plugins?` slots since D-110, and one alias per key since D-146; their own closure — `FeatureContext`, `AxisContribution`, `InsertionGeometry` and what those name — stays declared here at the middle tier. **What a tier decides is where a name is _declared_ — never what the compiler will let a consumer write, and never, on its own, what they can hoist.** An ordinary-tier consumer may author an axis installer **inline** _and_ hoist it into a `const hoistedAxis: AxisInstaller`, because the slot's own alias is published at their tier. What importing `sortable/feature.js` buys is the **lower-level named authoring vocabulary** — `FeatureContext`, `AxisContribution`, `InsertionGeometry` — for writing an installer's parts down and reusing them across a library, not the ability to construct the shape.

~~The installer aliases are **names without structure** at the _ordinary_ tier: a consumer writing only `sortable.js` can write `axis: y().axis`, cannot write `axis: (ctx) => ({ … })`, and never sees what one is.~~ **Retracted by D-78, and it was false when written.** TypeScript resolves a parameter's type structurally whether or not its alias is re-exported, so contextual typing hands a `sortable.js`-only consumer the full shape of `AxisInstaller`, its contribution group (`SortableContribution` when this was written; `AxisContribution` since D-146) and `InsertionGeometry`; the D-77 landing review reproduced it by compiling a file that imports nothing else. **The claim was a property of neither the type system nor the entry map**, and §The export topology already conceded the general form two thousand lines later — _a consumer who wants past it types one more import rather than defeating anything_ — while this section still asserted the strong one.
...
**Every callback slot is a named type alias, and that is normative rather than stylistic (F-51).** `onEnd?: SortableOnEnd`, never `onEnd?(result): void` and never an inline `onEnd?: (result) => void`. Two facts force it, and the compiled fixture found both:
...
type LandingContribution = Readonly<{
  landingTiming: LandingTiming; // middle-tier public (D-61, D-63, D-155)
  retire?: () => void;
}>;
...
**Implemented (D-151, superseding D-150).** The sentence below is true of where each slot is **declared** and was false of what the plugin position **accepts**, and the two were not the same claim (F-131, F-141). **The diagnosis moved once more** (F-143): what the reviews found is not two writers but **one writer at a position nothing reads**. Every unique slot is declared on exactly one non-plugin group, every non-plugin key is atomic and last-wins, and each such group _requires_ its unique member — so the count is already one before any check runs. The assembler reads `axis.insertion` and `landing.landingTiming` positionally and the plugin loop reads `retire` and the two displacement hooks, so an axis in `plugins` is installed, never dereferenced, and leaks its rect index because only the `axis` position registers `insertion.retire`. **So the rule is positional and the check is computed, not restated.** An installer may contribute only the slots its position is read for. The unique-slot set is _derived_ — a key a sibling group declares and the plugin group does not — and one mapped type over the `plugins` position, intersected into the `const`-generic entry signatures, replaces a violating entry with a refusal type carrying the key's name. **No group carries negative knowledge of its siblings**: D-150's `insertion?: never`, `landingTiming?: never` and `constrain?: never` are not written, and every group below states only what it contributes. No runtime arbitration returns. **Explicit widening is out of scope**: `const plugin: SortablePlugin = axisInstaller` forgets provenance and the call site cannot recover it. Record [`composition-check-claude.md`](../reviews/phase-23/composition-check-claude.md).

**One flat record became three groups at D-146, and every unique slot is now producible from exactly one config key.** `axis` produces a group whose `insertion` is required, `landing` one carrying `landingTiming`, and a `plugins` entry one carrying **multi-writer slots only** — so two writers on a unique slot are unrepresentable rather than detected, and `claim` with both `duplicate-contribution` identities is deleted. **`placeholder` left the contribution entirely rather than moving to a key**: the factory answers _what element_, the axis answers _where it belongs_, and the behavior joins them — a placement relationship, not ownership, and D-45's rule makes a slot with nothing to construct a config slot. **The rule is this section's own, applied one level down**: D-45 deleted `getHandle`, `getVisual` and `callbacks` because a slot with nothing to construct is a config slot; D-146 says a slot with exactly one possible producer _position_ is a keyed installer rather than a discovered contribution. Everything below about grouping, the absent discriminator, the paired capability and the receiver negative survives unchanged — what left is the discovery, not the bag.
...
**Implemented (D-147).** The **execution** order above is the guarantee and does not change; the **storage** order did. `retireHooks.reverse()` is deleted from both assemblers and every reader walks the array backwards — the construction unwind already did, D-39's placeholder undo already did, and the two operation-retirement loops joined them. What went is a second representation of one fact, normalized once per controller so a single consumer could iterate forwards (F-137). The receiver negative is unaffected: both retirement loops pass the hook as an argument to `unwind`, which calls it bare.

**Cleanup is recorded in installation order, and no reader is handed a normalized copy.** ~~before any claim runs … and the list is reversed exactly once~~ — `claim` is deleted (D-146) and so is the reversal (D-147); what the two bugs below taught is unchanged, because both are about **when a hook is recorded** rather than about which direction the array is read in:
...
**Total across construction, not merely within `assemble` (D-80 (b), F-69).** The sentence above was true of the assembler and false of the call that drives it: the collection was pulled and copied from inside `createSortableRuntime`, **after `assemble` had returned**, so a consumer-triggerable throw there left every recorded hook unrun and a kernel and realm already built by `draggable()` with nothing to destroy them, `arm()` never having been reached. **Normative:** the collection is pulled and copied **before the first installer runs**, and the copy is passed onward — so no consumer-triggerable throw remains between the first `retire` hook being recorded and the bracket that unwinds them.

**The motive is F-69 alone since 2026-08-25** (D-121, F-98). F-68's throw was `copyUniqueItems` refusing a collection containing one element twice, and that refusal is deleted: distinctness is published on `items` and a duplicate is outside the contract. What is untouched is the other reason, and it was always the load-bearing one — `merged.items()` is **consumer code** and can throw on its own. **The ordering is deliberate and must stay stated**, because it was previously supplied by argument-evaluation order alone: `items()` and `assemble(…)` sat as sibling arguments in one call, and only left-to-right evaluation kept a throwing `items()` from stranding every hook.
...
**What remains is one placement precondition**: an insertion anchor that is not in the placeholder's container, thrown inside the committed-move bracket and classified there. It is not a consumer-input check — the anchor is an element the library is moving relative to, and the throw guards a cross-container relocation of the library's own footprint (D-27).
...
- **The runtime argument is one small `SortableActivation` per operation**, created in `activation.effect` and cleared at retire. It exists because both feature views need a **non-null** `placeholder`, which a controller-lifetime runtime cannot promise before activation. It is written whole, once, and dropped whole — none of its members is rewritten and nothing is written per call, so no feature has to guard a null it can never see.

**The two views declare disjoint key sets, and that is the rule the pair carries.** Committed frame state is read off the frame; what the frame cannot supply travels on the runtime view. A field appearing on both is a mirror — two representations of one fact, kept in step by nothing — and the divergence is a defect whether or not a reader has yet observed it (D-177). `tests/sortable/feature.declaration.test.ts` asserts the intersection empty, with each side paired against its owner so a view cannot declare a member its owner does not supply — an **upper** bound, capping each view at what the behavior guarantees. The lower bound is the axis assignment itself: `InsertionGeometry`'s two slots are named aliases (D-194), so their parameters are checked contravariantly and a view narrowed past what a shipped axis reads reddens `y.ts` and `xy.ts` under `just typecheck`.
...
| `InsertionRuntimeView` | `live: () => boolean` | I-36, **re-grounded on D-37**. The candidate loop invokes `visual()`'s resolver once per candidate; that is a **declared consumer slot**, so it is one of the three acts D-37's finite domain still prohibits after logical closure. The loop is feature-private (D-19, H-4) and cannot reach the behavior's runtime, so the reading has to arrive as data. **Under D-38 the reading is the logical latch and nothing else** — `BehaviorContext`'s liveness member (D-53), never `presentation.signal.aborted`, which lags logical closure once teardown defers to the transaction boundary (D-36). |

**~~a growing structural contract~~ — corrected: the pair moves in both directions, and the fifth change was a move rather than a widening.** `snapshot` was removed from `InsertionRuntimeView` and added to `InsertionFrameView` in one act, because the frame commits it; `insertion` was removed from `InsertionRuntimeView` with no compensating widening, because the frame already declared it. Neither is a counter-example to the paragraph below — the behavior's existing per-operation object still satisfies the runtime view with no wrapper and no allocation — but _additive_ is not the rule, and a field is placed on the side that owns it rather than added to whichever side happens to need it next.
...
| `landing()` | ~~timing options and the WAAPI animation~~ — **timing options alone since D-155**; the animation, the property it claims, the vector and the handle are the kernel's | nothing. There is no handle to hold and nothing to release |
...
**`handle` carries an accessibility obligation it did not carry before** (D-46), and it carries it as a config key now. Scoping the drag to a grip resolves most of probe E's input-policy failures, but it is **not** the accessibility answer: it removes keyboard reordering unless the consumer makes the grip focusable, which is a stated consumer obligation rather than a nicety. The admission policy itself — the `[data-drag-ignore]` decline rule (~~the interactive/editable rule~~, withdrawn by D-129), `isComposing`, the modifier for plain-text selection — is 02's and 05's to state; this section records only that the `handle` slot is where a consumer meets it.
...
~~**The error the consumer receives is coarse (D-64).**~~ **It carries the stage (D-132).**
...
~~`DraggableErrorCode` was a four-member string union — `consumer`, `interaction`, `presentation`, `platform` — and the 13 `FAILURE_*` constants left the ordinary tier, with a **total** stage → code mapping becoming a library obligation that had to be total in the type rather than by convention.~~ **Deleted whole at D-132.** The obligation's purpose was that `code` could not mean different things in different behaviors; D-130 discharged that structurally by making the kernel the only constructor, and what remained of the mapping did one job — discard information — under a compile-time guard.

**The axis was the defect, not the coarseness.** A code named _whose fault_ and was derived from a stage, which names _where the library was standing_ — and D-81 had already settled that those are different questions. The proof is this package's own compiled demonstration: `tests/revision/revision-2.ts` showed the one decision the field existed for, `error.code === 'consumer' ? 'mine' : 'theirs'`, and a consumer's own garbage `bounds` source surfaces as `interaction`, `presentation`, `presentation` or `interaction` and **never** `consumer`. A coarse answer is right and imprecise; that one was precise and false.

**`null` is `panic`, and it means the controller is destroyed.** Not _unknown_ and not an _other_ bucket: `FailureStage` classifies faults **within** an operation, and a panic ends the controller, so there is nothing to classify. It replaces a `platform` code that made a panicked controller indistinguishable from a failed `requestAnimationFrame` (F-104).

~~**The human channel is the message.** `err.stage === 4` in a logged payload is worse than a word, so `DraggableError`'s constructed fallback names the stage in words — `drag: action prepare failure`.~~ **Withdrawn at D-133**, because the mechanism does not run where the symptom is. The message is the _cause's_ whenever a cause is an `Error`, which is every ordinary fault the library reports; the constructed fallback fires only when a consumer threw a non-`Error`, so the twelve words never reached the logged payload they were introduced for, and cost the shared root 115 B of its 261 to say something else (F-105). **The fallback carries the number** — `drag: failure at stage 4` — and the twelve published constants are how a consumer names it, which is the reason they are published.

**A twelve-member `code` union at the ordinary tier is still refused**, and D-133 leaves that refusal standing on its remaining ground. It was refused for creating a permanent synonym per stage that must never diverge — D-74 is this package's worked example — and the readability it bought was to be paid for in the message instead. There is no such payment now: the two candidates were always the same twelve strings in the same root behind the same class, separated only by the compatibility obligation, so refusing the expenditure removes the alternative rather than redirecting it.

~~`DragErrorContext` carried `domain: ReorderTransactionResult | null`.~~ Deleted by D-130; see below.
...
// Opaque since D-143: the arms are unpublished and the type is a brand, so a
// consumer builds a resolution and returns it and does nothing else with it.
declare const ReorderResolution: Readonly<{
  accept(): ReorderResolution;
...
The placeholder is the dragged item's authoritative layout footprint for the whole operation: created detached during `activation.prepare`, inserted as a post-commit effect, never duplicated or lost, valid until ~~the kernel's final pin~~ **the join's authoritative measurement** (D-166 deletes the write; the measurement is what the placeholder has to be valid for), and removed with presentation immediately afterwards — before anything interpolates.
...
type LandingOptions = Readonly<{
   ...
   * distance-scaled duration is expressible without owning the animation.
   */
...
type LandingTimingContext = Readonly<{
  /** Origin-relative deltas: where the visual was, and where the drop decided it belongs. */
  fromX: number;
  fromY: number;
  toX: number;
  toY: number;
...
**It costs nothing the thunk did not already cost.** The invocation site, the once-per-landing rule, the validation domain and the ordering against the reduced-motion collapse are all unchanged; **only the argument list grows**, from zero to one object. `from` and `to` are the two endpoints the kernel already holds at that moment, and `distance` is one `Math.hypot` — so the contextual form is strictly more expressive than the thunk at a cost that does not appear in any measurement.
...
~~**This is a tier move, not a lost capability.**~~ **D-155 makes it a deletion, and the price D-63 quoted is what got paid at every tier.** What D-63 removed from the ordinary tier was the **public lifecycle protocol** a consumer-supplied runner requires: exported types, a handle contract, a completion latch the consumer can call into, and a relinquishment obligation on `destroy()` that the consumer must honour or the join's own write is overridden. That protocol exists because the library **waits** for the animation — and D-155 stops waiting, so there is no completion to latch, no control to relinquish and no handle to contract about. A kernel-tier author supplies **timing**, on the same terms an ordinary consumer does. **A spring is still authorable and is now a consumer's own animation**: the tail claims an additive `translate` and composes with anything else on that property rather than replacing it.
...
Without this feature no landing module is imported and the visual is ~~pinned at the placeholder,~~ released at the placeholder and left there. **With it, the drop is released identically** (D-166: neither path writes a position) and an additive contribution decays away afterwards — so the two paths differ in what the element does after the operation ends, never in when it ends. `duration: 0` is one code path with the rest, and installs an animation of no length rather than skipping one.

`landing({ duration, easing })` supplies the timing the kernel's own Web Animations contribution uses, honouring `prefers-reduced-motion` by collapsing duration to zero. ~~`landing({ run })` replaces it entirely; a spring driving `requestAnimationFrame` and calling `done()` when it settles is a first-class citizen, because nothing in the contract assumes a CSS timing function or a finite known duration.~~ **Removed at the ordinary tier (D-63).** ~~The clause that follows it is still true and still load-bearing: **nothing in the contract assumes a CSS timing function or a finite known duration**, which is what keeps a kernel-tier spring authorable and what makes `duration: 0` safe.~~ **The seam assumes a CSS timing function, deliberately (D-155).** `LandingTail` is `{ duration, easing }` and the `easing` string is handed straight to `element.animate()` at the kernel tier, so what a behavior may author here is a duration in milliseconds and a CSS/WAAPI easing function — that is the whole payload, at every tier. **A spring is therefore not authorable through this seam, and is not meant to be**: an animation more expressive than an easing curve is a feature of its own, not a widening of a timing seam that carries two scalars. What the contract still does not assume is a **finite known duration**: nothing waits on the tail, so `duration: 0` is one code path with the rest and an unbounded duration is a cosmetic misuse rather than a hang.

~~**Synchronous completion is explicitly supported.**~~ **Nothing completes** (D-155). The reserve-seal-arm protocol existed so that a runner finishing inside `start` found a hold to release; with nothing waiting on the animation there is no completion to be early, late or duplicated.

**Timing is never responsible for correctness**, which is the surviving half of the same claim and is now structural. ~~The pin is the kernel's, taken against an authoritative measurement of DOM the serial order has already made final, and it lands **before** anything interpolates.~~ **The position is the kernel's and nothing writes it** (D-166): it is decided once against an authoritative measurement of DOM the serial order has already made final, and the visual reaches it by being released into flow **before** anything interpolates. A policy that returns a nonsense duration produces a nonsense transit and a correct drop; a policy that throws produces no transit and a correct drop.

~~`retarget?(target)` is optional and improves trajectory quality only.~~ **`retarget` is removed** (02, following D-41). Its stated purpose was smoothing a late readiness correction, and with one authoritative measurement taken after the authored commit there is no correction to smooth — a seam whose only justification was deleted is not kept on the chance a use appears. ~~A runner's obligations are therefore exactly two: call `done()`/`fail()`, and relinquish the transform on `destroy()`.~~ **A landing feature's obligations are none**: it answers a question and holds nothing.
...
**It is not a lifecycle gate, and structurally cannot become one:** ~~it has no access to `SettlementScope`, which is passed only to `settlement.effect`~~ — **there is no capability to reach** (D-155), so no seam it can be installed at suspends anything. An in-flight displacement never delays release, settlement or presentation teardown. This is a case where probe 2 turns a probe-1 _rule_ into an absence of capability.
...
| landing animation (`landing`'s timing; the interpolation itself is the kernel's) |
...
| `kernel.js` — `@ydinjs/drag/kernel`, **the kernel tier** (D-48, **rebuilt by D-68**) | **`draggable`**, the 12 **`FAILURE_*` constants** (D-64; a second publication point at `drag.js` since D-132), the 3 **`LIFT_*`**, the 5 **`SETTLED_*`**, **`AT_PROPOSAL`**/**`AT_CONSUMER`**, the 4 **`CANCEL_*` origins** (D-154), the 8 **phase constants** — `1 + 12 + 3 + 5 + 2 + 4 + 8` = **35 values** (~~36~~ — F-174: the sentence disagreed with the list it is the total of), ~~**`toDraggableError`**~~ deleted by D-132 | **`BehaviorFactory`**, **`BehaviorContext`** (~~`KernelHost`~~, renamed by D-170 §The behavior-facing interface), **`BehaviorSpec`**, **`FailureStage`**, **`CancelOrigin`**, and the rest of `BehaviorFactory`'s structural closure, enumerated in [02](02-kernel-behavior-contract.md) §The kernel tier's public vocabulary — **35 types**, `12 + 23` (33 until 2026-08-22; `BehaviorLiftSession` and `InheritedSpace` were each ratified separately, by 07 §K-1 and D-85, and neither updated the total, so 35 was one high until `CancelOrigin` made it true — F-174) |
| `sortable.js` — returns a `SortableController`, takes `root` (D-48) | `sortable`, **`ReorderResolution`**, **`AT_PROPOSAL`**, **`AT_CONSUMER`**, the 4 **`CANCEL_*` origins**, and **`CANCEL_ITEM_REMOVED`**/**`CANCEL_COLLECTION_INVALIDATED`** — the behavior's own two supplied reasons, published beside the result that carries them (D-154) | **`SortableConfig`** and every alias it names — `ItemSource`, `OnReorder`, **`SortableOnStart`**, **`SortableOnEnd`**, **`SortableOnDragError`** (qualified by D-109), `ResolveHandle`, `ResolveElement`, `PlaceholderFactory`, **`PlaceholderContext`**, **`AxisInstaller`**, **`SortableLandingInstaller`**, **`SortablePlugin`** (published by D-110, one alias per key since D-146) — plus `ReorderRequest`, `ReorderProposal`, `CollectionSnapshot`, `ReorderResolution`, ~~`AcceptedReorderResolution`, `RejectedReorderResolution`~~ (unpublished by D-143 — the resolution is opaque), `AcceptedReorderResult`, `NoopReorderResult`, `RejectedReorderResult`, `CanceledReorderResult`, **`ReorderTransactionResult`**, `CancelStage`, `CancelOrigin`, ~~**`SortableErrorContext`**~~ (renamed from `DragErrorContext` by D-75, deleted by D-130), `SortableController` |
| `sortable/feature.js` — **the middle tier** (D-61) | **`insertionAt`** — one value, added 2026-08-25 (D-123, D-125), which makes this a **runtime** entry | **`AxisInstaller`**, **`SortableLandingInstaller`**, **`SortablePlugin`**, **`FeatureContext`**, **`AxisContribution`**, **`LandingContribution`**, **`SortablePluginContribution`**, **`InsertionGeometry`**, **`ResolveInsertion`**, **`MovedInsertion`** (D-194), **`DisplacementHook`**, the consumer-declared view types an installer reads, and — **as re-exports since D-68, declared at the kernel tier** — ~~`LandingStart`, `LandingContext`, `LandingHandle`~~ `LandingTail` (D-155), `Disposer` |
| `sortable/y.js` | `y` | — |
| `sortable/xy.js` | `xy` | — |
| `sortable/landing.js` | `landing` | `LandingOptions` |
| `sortable/layout-animation.js` | `layoutAnimation` | `LayoutAnimationOptions` |
| `free-drag.js` — **the second behavior's ordinary tier** (D-69) | `freeDrag`, **`FreeDragResolution`**, `AT_PROPOSAL`, `AT_CONSUMER`, the three **`LIFT_*`** constants (D-141) and the 4 **`CANCEL_*` origins** (D-154) | **`FreeDragConfig`** and every alias it names, `FreeDragController`, `FreeDragSubject`, `FreeDragRequest`, `DragGeometry`, `DragAxis`, **`LiftMode`** (~~**`FreeDragLift`**~~ — D-141), `FreeDragTransactionResult` and its three arms, ~~the two resolution members~~ (unpublished by D-140 — the resolution is opaque), ~~**`FreeDragErrorContext`**~~ (deleted by D-130), `CancelStage`, `CancelOrigin` |
| `free-drag/feature.js` — **its middle tier** (D-70) | — | **`ConstraintInstaller`**, **`FreeDragLandingInstaller`**, **`FreeDragPlugin`**, **`ConstraintContribution`**, **`LandingContribution`**, **`FreeDragPluginContribution`**, **`MotionConstraint`**, `ConstraintView`, `MotionDraft`, and — as re-exports — `FeatureContext`, ~~`LandingStart`, `LandingContext`, `LandingHandle`~~ `LandingTail` (D-155), `Disposer` |
...
- **`free-drag/landing.js` duplicates an entry, not an implementation.** The landing policy is behavior-neutral — ~~`LandingStart`, `LandingContext` and `LandingHandle` are kernel SPI (D-68)~~ **and since D-155 there is nothing behavior-specific left to be neutral about**: the timing domain and its reduced-motion collapse are one internal module both entries wrap, and the interpolation is the kernel's. What is not shared is the _installer type_, because the contribution types differ, and unifying those is F-64's deferred question rather than Phase 18's. The precedent is `rect-index.ts` shared between `y()` and `xy()` at a measured **60 B**, recorded rather than absorbed; Phase 21 measures this one the same way.
...
- **`sortable/feature.js` is new** (D-61). It is the ladder's second rung and ~~it has **no runtime exports at all** — every name on it is erased. That is the honest measurement statement for this entry: it cannot demonstrate absence because it contains nothing present, and unlike the three subpaths D-56 deleted for exactly that reason, it is not pretending to. It exists to give the authoring types an address, not to isolate a cost.~~ **It gained one runtime export on 2026-08-25** (D-123, D-125) and is now a `runtime` entry in `files.json` with a `default` condition in the export map.

  **`insertionAt` is there because the tier publishes an obligation it did not implement.** Exactly one slot on exactly one contribution group _produces_ an `Insertion` — `AxisContribution.insertion` — so the sole third-party producer is the axis; D-119 gave `Insertion` one construction owner and proved five spellings are one rule, and the package then kept that owner on its own side of the boundary D-61 opened. The rule is derivable from `Insertion`'s published documentation in four lines, which is why the seam that receives the gap now checks nothing (D-123) — but deriving it is exactly how one expression came to be written in four modules (F-91). **Free drag's middle tier still has no runtime exports**, which is what keeps the `runtime`/`typeOnly` split in `files.json` load-bearing rather than vestigial.

- ~~**`FailureStage` and the 13 `FAILURE_*` constants move from `drag.js` to `kernel.js`** (D-64), because they are how a **behavior** classifies and the ordinary consumer receives a coarse code instead.~~ **D-132 returns them, without taking them away from `kernel.js`.** They are still how a behavior classifies _and_ they are now what `DraggableError` carries, so there are **two publication points over one declaration** in `src/kernel/failures.ts` — the same shape `AT_PROPOSAL`/`AT_CONSUMER`/`CancelStage` — and, since D-154, `CancelOrigin` and its four constants — already run between `kernel.js` and `sortable.js`. Not on the behavior entries: that would split a vocabulary so the consumer named the type at one entry and the values at another.
- **`DraggableError` is new on `drag.js`** (D-64), and it is why that entry still exists — see below. ~~And `DraggableErrorCode`~~, deleted by D-132.
- ~~**`LandingStart`, `LandingContext` and `LandingHandle` leave `sortable/landing.js` for `sortable/feature.js`** (D-63, D-61). They stop being consumer vocabulary and stay authoring vocabulary.~~ **All three are deleted by D-155**, and what `sortable/feature.js` publishes in their place is `LandingTail` and `LandingTiming` — timing, with no lifecycle attached. `sortable/landing.js` keeps `LandingOptions`, which is now `{ duration?, easing? }`.
...
~~**That argument is void and this entry survives on a different one (D-64).** `onError` no longer hands the consumer a `FailureStage`; it hands a `DraggableError` carrying a coarse `code`.~~ **The struck argument is restored by D-132, and the entry now rests on both.** `onError` hands the consumer a `DraggableError` whose `stage` **is** a `FailureStage`, so those names are structurally public at the ordinary tier again — exactly as the doubly-struck text above said before D-64 made it false. What does _not_ return is the inversion it warned about: the stages reach that consumer from **this** root, not from `kernel.js`, so no ordinary consumer imports the kernel to `switch` on an error its own handler was given. **D-64's tier half stands and its vocabulary half reverses** — which is the cleanest possible outcome for a document that recorded both halves of the argument instead of only the winning one.
...
- **The stage constants are runtime exports.** §The public/internal boundary already called them public — a consumer receiving `onError` or a canceled result has to discriminate them — but the runtime column listed only `draggable` and `sortable`, so the type shipped and the values did not. A numeric union whose members are unnameable is not a public type. **The rule survives D-64 and its subject changes tier**: the constants are still runtime exports, now from `kernel.js`. ~~And `DraggableErrorCode` needs none because its members are string literals a consumer writes directly.~~ **D-132 deletes that union and applies the rule a second time, at the tier the sentence above was originally written for**: `DraggableError.stage` is a `FailureStage`, so the twelve constants are runtime exports of `drag.js` as well. `CODE_OF_SIZE` §4 permits exported numeric failure constants _when they are part of the supported consumer contract_, and this is the decision that makes them so — a later size pass must not read them here as an unowned publication.
...
~~**One row's reasoning survives intact and is why the rule is not "never throw":**~~ **superseded 2026-08-25 (D-124), and it is the row that made the point.** An unbounded **landing** duration is still the one value the platform accepts and never completes, and the landing still holds the settlement gate, so the operation still hangs with no terminal at all — a hang being the single failure this architecture cannot classify. **All of that is a description of what the library goes on to do with a value the contract never admitted**, which `CODE_OF_SIZE.md` §1.1 now reaches only for states a conforming integrator can produce. The gate closes at reachability and the check is deleted. What the row does still illustrate is the _contrast_: `layoutAnimation` takes the same option, accepts the same bad value, holds nothing — the two now differ in consequence and no longer in machinery, since neither checks anything.
...
~~The reduced-motion collapse does not change this, and the **ordering stays as Checkpoint D repaired it**: resolution and the `Infinity` test both precede the collapse, so a thunk returning `Infinity` is refused under `prefers-reduced-motion: reduce` exactly as it is without.~~ **The test the ordering protected is gone (D-124), so the guarantee is gone with it**: the collapse is now the first thing the resolved value meets, and under `reduce` an unbounded duration collapses to zero and lands normally while without the preference it hangs the gate. **A consumer diagnosing this bug does get a different answer because of the reader's OS setting** — the price of the deletion, recorded here rather than discovered later, and pinned in `tests/sortable/features.browser.test.ts` — _should land an unbounded thunk result under a reduced-motion preference_. The ordering itself is unchanged; only the thing it ordered is. What follows was written of the deleted test and is kept as its reasoning — a consumer diagnosing a bug must not get a different answer because of the reader's OS setting. ~~a consumer whose thunk throws or returns `NaN`~~ — a throw still classifies at that moment; a `NaN` result is no longer the library's business and reaches `animate()`'s own throw when the collapse does not preempt it.
...
- `landing({ duration })` as a **contextual function** is the one settle-time domain. It is called once per landing; ~~its result is tested for `Infinity` only~~ **its result is tested for nothing since D-124**, and a throw or a value `animate()` refuses ~~is classified as a landing failure at that moment~~ **is reported as a warning that changes nothing** (D-155): the drop is already decided, committed and released by the time the timing is asked for. This is the parity shape for the shipped `landingTiming()` (ledger §2, L-6), and D-67 is what keeps that parity after D-63 removed the runner it used to be reachable through.
...
**Internal and unstable at every tier** — not exported, and free to change without notice: `SortableSlots`, **`Behavior` and its brander** (D-55), and the outcome/recovery constants. ~~the phase/lift/outcome/recovery constants~~ — **D-68 publishes the phase and lift constants at the kernel tier**; the outcome and recovery constants are the sortable behavior's own and stay here. Also internal, and named because the rule that keeps them here is worth stating once: the seam driver (`SeamOutcome`, `SEAM_*`, `SeamDriver`), the frame transaction (`FrameTransaction`), the full `Lifetime`, the frame helpers, the lift acquisition, the reporter, the invalidation utilities and the protocol event names — **the kernel never hands one of them to a behavior and never accepts one from it**, which is the discriminating test.

**Published at the middle tier** (D-61, D-146): the three installer aliases, the three contribution groups, `FeatureContext`, `InsertionGeometry`, its two slot aliases `ResolveInsertion` and `MovedInsertion` (D-194), `DisplacementHook`, and the landing seam types D-63 moved here. Every one of them was on the list above until Revision 2.1, and moving them is the whole of D-61's cost: **they acquire a versioning promise.** By this table's closure rule, publishing them publishes what they structurally name, so the minimum middle-tier surface is whatever the three contribution groups and `FeatureContext` reach — narrowing it means narrowing those declarations, exactly as D-48 records for `BehaviorSpec`. `SortableSlots` stays internal and is the reason the closure stops where it does: an installer _returns_ a contribution and never sees the flattened record the behavior builds from it.

**Internal to the ordinary tier, published at the kernel tier** (D-48, **enumerated by D-68**): `BehaviorSpec`, ~~`KernelHost`~~ `BehaviorContext` (D-170 §The behavior-facing interface), `BehaviorFactory`, `BehaviorInstall`, `Transition`, `ReleaseTransition`, `SettlementTransition`, `ActionTransition`, `CommandAdmission`, `AdmissionSubject`, `SettlementInput`, ~~`SeamRejection`~~ (D-152), `ActivationScope`, ~~`SettlementScope`~~ (D-155), `LifetimeScope`, `Disposer`, `Draft`, `Frame`, `KernelFrame`, `OperationIdentity`, `FramePartOf`, `VisualLiftSession`, `LiftMode`, `Phase`, `OffsetBox`, `PreparedSettlement`, `ResolutionCommand`, `CancelStage`, `CancelOrigin`, ~~`LandingStart`, `LandingContext`, `LandingHandle`~~ `LandingTail` (D-155), `FailureStage`. A `sortable.js` consumer can name none of them; a `@ydinjs/drag/kernel` consumer must be able to name enough of them to return `{ spec, controller }` from D-47's factory. **Two names were on this list and are struck**: `SeamOutcome` and `ArmOutcome` are driver-internal, in no public closure, and were listed by assumption rather than by derivation — which is exactly the failure mode D-68's rule exists to stop. `CancelStage` and the landing type **are** on it now and are re-exported at the tiers that also need them. **`SettlementScope` was on it and is deleted outright** (D-155): the settlement seam hands the behavior no capability at all.
...
**Public and stable at the ordinary tier:** everything in the `sortable.js` and the built-in `sortable/*` rows above — including `SortableConfig`, which Revision 2 adds — plus `drag.js` in full: `DraggableError`, `DraggableWarning`, `FailureStage` and the twelve `FAILURE_*` constants (D-132), `Point`, `DOMRealm`. ~~`DraggableErrorCode`~~ is deleted. `CancelStage` and `AT_PROPOSAL`/`AT_CONSUMER` **remain exported from `sortable.js`, where they always were, and their declaration site moves to the kernel tier** (D-68); `CancelOrigin` and its four constants are declared and published on exactly that route from the day they land (D-154): the kernel produces the stage on a `canceled` settlement input and a behavior writes one into D-66's fallback, so a kernel-tier author had no route to them except an import from the sortable. Nothing changes for an ordinary consumer — same specifier, same type.

~~**`FailureStage` and the `FAILURE_*` constants are no longer on that list (D-64).** The struck reasoning was that a consumer receiving `onError` has to discriminate them; it now receives a coarse `code` instead, so it discriminates that. Note what this costs, since the earlier text argued the other way with the same rule: a consumer loses the ability to distinguish a placeholder-move failure from a renderer-write failure, which was real telemetry. The owner's judgement is that it was granular on the pipeline axis and silent on the fault-attribution axis, and that only the second is actionable.~~

**Both are back on that list at D-132, and the cost this paragraph priced is refunded.** The rule never moved — export what a public type structurally depends on — and its input moved twice: `DraggableError.stage` is a `FailureStage`, so the ordinary tier's public types depend on a stage again. The judgement that reversed is the one about the axes. The pipeline axis is not _silent_ on attribution and the fault axis is not _actionable_; the second was **derived from the first** by a mapping D-81 had already shown cannot carry attribution, so what shipped was a granular fact projected onto a promise it could not keep. A consumer distinguishing a placeholder-move failure from a renderer-write failure is exactly what returns. **A consumer who genuinely needs the finer signal has no supported route to it** — that is the cost, stated rather than absorbed, and its falsifier is in 00 §What would falsify this model.
...
**The settlement primitives stay internal, and they now stay internal for a stronger reason.** ~~`SettlementScope`~~, `PreparedSettlement` and the settlement attempt were unreachable through D-33's protocol; after D-41 there is no protocol to be unreachable through, and after D-155 the scope itself is deleted — a capability that carries nothing is not published, it is removed. The consumer's half of the authored-presentation problem is now the consumer's own `await`.
