...
**Unimplemented (D-150).** The sentence below is true of where each slot is **declared** and was false of what the plugin position **accepts**, and the two were not the same claim (F-131, F-141). A group's cardinality is enforced by what it **requires**; every unique group requires its slot and is closed, while the multi-writer group requires nothing and therefore refused nothing — a hoisted `AxisInstaller`, `ConstraintInstaller` or landing installer assigned to the plugin alias compiled, and an axis placed in `plugins` leaked its rect index because only the `axis` position registers `insertion.retire`. The fix is one clause per refused slot on each multi-writer group — `insertion?: never`, `startLanding?: never`, `constrain?: never` — defended by a declaration instrument that asserts the non-assignability. No runtime arbitration returns.
