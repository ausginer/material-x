...
**Implemented (D-151, superseding D-150).** The sentence below is true of where each slot is **declared** and was false of what the plugin position **accepts**, and the two were not the same claim (F-131, F-141). **The diagnosis moved once more** (F-143): what the reviews found is not two writers but **one writer at a position nothing reads**. Every unique slot is declared on exactly one non-plugin group, every non-plugin key is atomic and last-wins, and each such group _requires_ its unique member — so the count is already one before any check runs. The assembler reads `axis.insertion` and `landing.startLanding` positionally and the plugin loop reads `retire` and the two displacement hooks, so an axis in `plugins` is installed, never dereferenced, and leaks its rect index because only the `axis` position registers `insertion.retire`. **So the rule is positional and the check is computed, not restated.** An installer may contribute only the slots its position is read for. The unique-slot set is _derived_ — a key a sibling group declares and the plugin group does not — and one mapped type over the `plugins` position, intersected into the `const`-generic entry signatures, replaces a violating entry with a refusal type carrying the key's name. **No group carries negative knowledge of its siblings**: D-150's `insertion?: never`, `startLanding?: never` and `constrain?: never` are not written, and every group below states only what it contributes. No runtime arbitration returns. **Explicit widening is out of scope**: `const plugin: SortablePlugin = axisInstaller` forgets provenance and the call site cannot recover it. Record [`composition-check-claude.md`](../reviews/phase-23/composition-check-claude.md).
