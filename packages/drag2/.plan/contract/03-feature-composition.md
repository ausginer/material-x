...
**The human channel is the message.** `err.stage === 4` in a logged payload is worse than a word, so `DraggableError`'s constructed fallback names the stage in words — `drag: action prepare failure` — while `stage` stays the machine channel. The split is explicit rather than one string union trying to be both, which is what a twelve-member `code` union at the ordinary tier would have been; that candidate is refused because it creates a permanent synonym per stage that must never diverge, which D-74 is this package's worked example of.
