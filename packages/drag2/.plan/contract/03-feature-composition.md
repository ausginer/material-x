...
The placeholder is the dragged item's authoritative layout footprint for the whole operation: created detached during `activation.prepare`, inserted as a post-commit effect, never duplicated or lost, valid until the kernel's final pin, and removed with presentation immediately afterwards — before anything interpolates.
...
type LandingTimingContext = Readonly<{
  /** Origin-relative deltas: where the visual is, and where it was pinned. */
  fromX: number;
  fromY: number;
  toX: number;
  toY: number;
...
Without this feature no landing module is imported and the visual is pinned at the placeholder, released and left there. **With it, the drop is pinned and released identically** and an additive contribution decays away afterwards — so the two paths differ in what the element does after the operation ends, never in when it ends. `duration: 0` is one code path with the rest, and installs an animation of no length rather than skipping one.
...
**Timing is never responsible for correctness**, which is the surviving half of the same claim and is now structural. The pin is the kernel's, taken against an authoritative measurement of DOM the serial order has already made final, and it lands **before** anything interpolates. A policy that returns a nonsense duration produces a nonsense transit and a correct drop; a policy that throws produces no transit and a correct drop.
