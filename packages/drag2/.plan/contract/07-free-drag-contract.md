...
| **D-35** | `LandingContext.from` is the delta the lift session last rendered | Was `from: { x: current.pointerX - current.originX, … }`, with the session recording nothing. **Now `VisualLiftSession.renderedX`/`renderedY`, written by `write` and sampled once at arm**; the behavior holds a `BehaviorLiftSession` carrying neither of them nor `dispose` |
...
| **K-2** | `VisualLiftSession` records what `write(x, y)` composed; `LandingContext.from` reads it (D-35). The behavior is handed a projection carrying `visual`, `baseTransform`, `compose`, `write` and **not** `renderedX`/`renderedY` or `dispose` — I-34's structural half, typed and asserted with `@ts-expect-error` |
...
| **K-2** | `VisualLiftSession.renderedX`/`renderedY`, written by `write` _after_ the assignment so a `FAILURE_RENDERER_WRITE` cannot leave the record claiming a delta the element never took; `BehaviorLiftSession` as a positive `Pick`; `ActivationScope.lift` and `moved`'s second argument both projected. Assertions: `tests/kernel/spec.declaration.test.ts` §the lift capability, including the converse row that both members survive on the kernel's own session — a `Pick` that picked nothing would satisfy every `@ts-expect-error` and prove nothing |
