...
| `home` is a function | ″ | `anchorTarget` (`:1423`) | ~~`FAILURE_LANDING_TARGET` (12) → `presentation`~~ — **a `DraggableWarning` since D-130**, the stage having been deleted with the tier that produced it; the landing is skipped, the drop stands (D-49) |
