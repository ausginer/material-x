...
| **K-2** | `VisualLiftSession` records what `write(x, y)` composed; the settlement reads it once, before the pin (D-35). The behavior is handed a projection carrying `visual`, `baseTransform`, `compose`, `write` and **not** `rendered` or `dispose` — I-34's structural half, typed and asserted with `@ts-expect-error` |
