...
**Where the refusal landed, while it existed: `FAILURE_LANDING_CREATE` (10) → `presentation`.** The check runs inside the library's `LandingStart`, which the kernel wraps at exactly that stage (`kernel.ts:1487-1497`) — the same slot `animate()`'s own throw for `NaN` already produces. One predicate, no second path.
...
| **B-8** | The frame part is five fields and `FramePartOf` rejects a kernel key in it. ~~`validateFramePart` rejects one at `arm()`.~~ **There is no runtime check** (D-124, D-122): the type is the whole of the contract |
