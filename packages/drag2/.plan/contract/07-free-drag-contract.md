...
**Neither half of that obligation is discharged, and D-90 is re-opened for both** (CE1-03, third pass). The **source** fix is real and is in the tree: `apply`, `invalidate` and `retire` are lifted off the record once and all three sites call them detached. What is missing is everything that makes the fix legible and durable.

- **(a) The sentence is not on the type.** `MotionConstraint` says nothing about detachment, bare functions or `this`, and neither does `InsertionGeometry`. The only source-side statement of the convention lives in the implementation file that already obeys it — and that comment asserts the missing artifact directly, claiming _the convention is stated on `MotionConstraint`_ when it is not (C-1). A contract section is a real improvement over nothing, but the reader this decision names is the **third-party installer author reading the published middle-tier type**, and that reader still meets silence. The _two conventions in one package_ argument is what makes this load-bearing rather than tidy: one convention stated in neither tier is not what that argument buys.
- **(b) The instrument does not measure the fix.** The row mapped to D-90 builds a constraint whose three members **ignore `this`**, so a bound call hands them a receiver they never read and the row passes identically either way. Reverting **all three** sites to bound — the maximally non-conforming tree this decision exists to forbid — leaves the suite green at 1004 passed with no type errors. The row is labelled discriminating beside a control honestly labelled non-discriminating under F-74; it is the second kind, not the first.
