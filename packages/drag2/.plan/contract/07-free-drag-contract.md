...
**Two holes are accepted and stated** (D-138). A brand on a parameter cannot bind a function that **declares no parameter**, so `() => ({})` stays assignable to both — correctly, since an empty contribution is valid for either behavior. And an **unannotated hoisted literal** is decided by structural assignability on its return type alone, so one carrying `{ placeholder, retire }` reaches free drag's `plugins` again and is discarded by an assembler reading three slots. That is CE1-01's residue returning, deliberately: it is the price of record independence, since the only thing that ever closed it was this record naming `placeholder`.
