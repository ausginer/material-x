...
**The two survivors are a different matter, and this document does not settle them.** Both were kept on the second question — `moveTo`'s coordinates are folded into committed frame state and poison every later geometry the library hands out; the landing's `Infinity` hangs a gate the library holds, which is the one failure this architecture cannot classify because classification needs something to happen. **Under the decisive form that reasoning is the _second_ half of the answer and the first half is unasked**: a finite coordinate and a finite duration are both stated domains in the table below, so on a plain reading each bad value is outside the published contract and the gate closes on it before ownership is reached. **What the two rows genuinely turn on is therefore a contract question, not a byte one** — whether those domains are published preconditions an integrator can meet and find, or whether the surface admits the value and the library merely dislikes it — and it is put here rather than answered because answering it changes the contract and belongs to the audit that reads it. **Nothing below is changed by recording the question**; the table states the position as decided.
...
| **K-6** | M-1 and M-3 re-measured for K-2's two scalar writes per sample, under the reproducibility standard 05 §Measurements (landed 2026-08-02) sets. The sortable suite green throughout |
