...
| `visual` is a function | ″ | **admission**, in both behaviors (D-84): free drag calls the resolver inside `admit`, the sortable inside `seedDraft`, which is admission's second half | `FAILURE_ADMISSION` (1) → `consumer` |
| `onStart` is a function | ″ | activation (`:1166`) | `FAILURE_ACTIVATION` (2) → `interaction` |
...
| `bounds(source)` shape | The feature resolves **lazily, on the first `apply` after a staleness mark** (D-81); `invalidate()` is a staleness flag and calls nothing. Garbage therefore surfaces at **the seam that next renders**, and the first of those is activation, because the rect starts stale | `activation.effect` first; thereafter `moved`, a `TAG_POSITION` effect, or `release.prepare` | `FAILURE_ACTIVATION` (2) → `interaction` first; thereafter `FAILURE_RENDERER_WRITE` (3) or `FAILURE_ACTION_EFFECT` (5) → `presentation`, or `FAILURE_RELEASE` (9) → `interaction`. **Never `FAILURE_ACTION_PREPARE`** |
| `moveTo(point)`'s coordinates — **a check, not a former one** (D-91) | **Added rather than deleted, and it is the only row here that is.** The value is not a slot the library reads once: it is folded into `offsetX`/`offsetY`, which are **committed frame state**, so a single non-finite coordinate poisons every later `deriveMotion`, every geometry object handed to the consumer, and the accepted `anchorTarget` the kernel pins with. That is library state corruption, not the consumer breaking only their own code | `action.prepare`, before anything is written to the draft | **none, and the action is discarded** — `prepare` returns `null`, so no offset is written and no effect runs; the misuse is surfaced on the platform reporter. A malformed `point` — `null`, missing fields, throwing accessors — is _not_ checked and throws naturally at the read, reaching `FAILURE_ACTION_PREPARE` (4) → `presentation` |
...
type FreeDragContribution = Readonly<{
  ...
  retire?: Disposer;
  /* Mutual exclusion, not slots — D-88 §Key-set totality. */
  insertion?: never;
  placeholder?: never;
...
**The rule is key-set totality, not one exclusion per direction** (D-88, superseding D-87's sufficiency claim). **Both contribution records declare the same key set**; a key a behavior does not implement it declares `?: never`. Today that is seven keys — `insertion`, `placeholder`, `startLanding`, `beforeInsertionMove`, `afterInsertionMove`, `retire`, `constrain` — of which free drag implements three and excludes four, and the sortable implements six and excludes one. **The boundary is asymmetric because the behaviors are**, and writing it as _one per direction_ described a symmetry the slot sets do not have.

**Enforced mechanically, because discipline is what failed here.** `tests/composition.declaration.test.ts` asserts `keyof SortableContribution` and `keyof FreeDragContribution` are **the same key set** — a single type-level equality that holds if and only if every slot of each is declared on the other, really or as `never`. A slot added to one contribution without its twin fails that assertion at the moment it is added, so D-87's stated maintenance obligation stops being an obligation and becomes a check.
...
**The lesson is the reusable half, and it is F-74's, arriving from the reviewer's side**: the first probe of the open case was _refused_, and refused for the wrong reason. A negative control that passes by a mechanism other than the one under test is indistinguishable from a working boundary. Any fix here must be pinned by an **unannotated** counter-example per open slot, driven through `freeDrag(item, config, { plugins: [...] })`, or the regression is unpinned exactly where it already escaped once. Every real slot on both contributions is optional, so the two records were mutually assignable and an `AxisInstaller` could be dropped into free drag's `plugins`, whose assembler reads `constrain`, `startLanding` and `retire` and **silently discards `insertion`** — a supported middle-tier API accepting a value and doing nothing with it. One optional `never` in each direction breaks it: `SortableContribution` declares `constrain?: never`, this one declares `insertion?: never`, and each names the **defining capability of the other behavior** rather than an arbitrary marker.

**Two lines, and they are the minimum that works.** Assignability is decided on the declared alias, not on what a particular function body returns, so one exclusion per direction refuses every cross-behavior installer including the `AxisInstaller → FreeDragInstaller` case, where `insertion` is required. **No runtime brand, no phantom discriminant, no unified contribution type**: a brand costs a field on the hot construction path and states nothing about _why_ the two are incompatible, while `insertion?: never` says exactly the true thing — _a free-drag contribution has no insertion, and an object that has one is not one._

**What stays assignable is correct.** An installer returning a bare `{}` still satisfies both, because an empty contribution genuinely is valid for either behavior; refusing it would be a boundary drawn for its own sake. `FeatureContext` identity, the shared landing runner and `LandingOptions` are untouched (F-64, B-7) — the incompatibility is in what an installer **produces**, never in what it is handed.

**The maintenance obligation is stated rather than discovered**: a new single-writer slot on either contribution needs its `never` twin on the other, or the boundary silently reopens for that slot. That is a real cost, and it is also the useful question — adding a slot now forces _is this cross-behavior?_ to be answered once, in the type, instead of being assumed. A third behavior turns this from two lines into a growing matrix, and **that** is the point at which an explicit common contribution contract earns its keep; it does not earn it at two.
...
**A contribution's members are invoked detached, and an author may not depend on `this`.** The behavior may lift `apply`, `invalidate` and `retire` out of the record and call them as bare functions, so a constraint written as a class instance or with `this`-reading methods is **outside contract** — it must close over its state, as the first-party `bounds()` already does.

Stated because the tree is currently split three ways — `apply` and one `invalidate` site bound, a second `invalidate` site and `retire` detached — so a `this`-reading constraint works from `controller.invalidate()` and throws on the first scroll and at every retirement. Nothing fails today only because the one shipped constraint closes over its state, which is exactly the non-discriminating control F-74 names.

**Detached is chosen rather than bound**, for the reason [03](03-feature-composition.md) §Assembly already gives for the sortable's identical flattening: a lifted member is one property read and one call at the seam instead of a record read plus a member read plus a bound call, and the sortable flattens all four of its contributed functions this way. Making free drag bound would leave a middle-tier author facing **two conventions in one package** — the worse outcome, since the same author writes against both tiers. The obligation this creates is one sentence on the type, not machinery.

~~Neither half of that obligation is discharged, and D-90 is re-opened for both~~ **— both halves landed on 2026-08-18, and the two findings are kept below because what they diagnose is worth more than the fact that it is fixed** (CE1-03, third pass). The **source** fix was already real: `apply`, `invalidate` and `retire` are lifted off the record once and every site calls them detached. What was missing was everything that makes the fix legible and durable, and it is what follows.

- **(a) The sentence is not on the type.** ~~Was.~~ **Landed:** `MotionConstraint` now carries the convention, the reason it is stated on the published declaration rather than at the call sites, and the _two conventions in one package_ argument that makes it load-bearing. `InsertionGeometry` still carries none — the sortable's flattening obeys the same convention and states it nowhere, which this section records as **out of D-90's scope and open**, not as closed. `MotionConstraint` says nothing about detachment, bare functions or `this`, and neither does `InsertionGeometry`. The only source-side statement of the convention lives in the implementation file that already obeys it — and that comment asserts the missing artifact directly, claiming _the convention is stated on `MotionConstraint`_ when it is not (C-1). A contract section is a real improvement over nothing, but the reader this decision names is the **third-party installer author reading the published middle-tier type**, and that reader still meets silence. The _two conventions in one package_ argument is what makes this load-bearing rather than tidy: one convention stated in neither tier is not what that argument buys.
- **(b) The instrument does not measure the fix.** ~~Did not.~~ **Landed:** the fixture's members record the receiver they are handed instead of ignoring `this`, and each site is driven **alone**, so a single bound site fails a single row. Four rows for three members, because `invalidate` is reached from both the scroll/resize listener and `TAG_POLICY`. The row mapped to D-90 builds a constraint whose three members **ignore `this`**, so a bound call hands them a receiver they never read and the row passes identically either way. Reverting **all three** sites to bound — the maximally non-conforming tree this decision exists to forbid — leaves the suite green at 1004 passed with no type errors. The row is labelled discriminating beside a control honestly labelled non-discriminating under F-74; it is the second kind, not the first.

**The falsifier is written down, and closure is measured against it rather than against a fixture shape.** Return all three call sites to bound (`constrain?.apply(…)`, `guarded(() => constrain!.invalidate())`, `constrain?.invalidate()`) and delete the two lifted `const`s: **the suite must not stay green.** How that is achieved is open — a member that reads `this` is the obvious route and not the only one — but a fixture whose members ignore `this` cannot do it, whichever way it is driven. The comment in `spec.ts` must also become true, or be reworded to cite where the statement actually lives.

**The falsifier was run, and it is recorded here as run rather than as satisfied by inspection.** Reverting the three `spec.ts` sites exactly as written above fails _should hand the apply site no receiver_, _should hand the scroll invalidator no receiver_ and _should hand the policy invalidator no receiver_; binding the assembler's `retireHooks.push(contribution.constrain.retire)` fails _should hand the retire hook no receiver_. The route taken is the obvious one the paragraph allows — members that read `this` — and the production tree was restored and re-verified before the marker moved. The `spec.ts` comment is true now and records that it was false when written.

**This is the same class as the finding it answers.** CE1-03 was _a convention nothing states_; its remediation is _a convention stated in the wrong place and a fix nothing pins_. Recording it as implemented on the strength of the contract section alone is how F-63 happened, and the deferred-decision table exists for exactly this state.
...
| `home` | `anchorTarget` | Latch read before. Single call. **Its result is read, finiteness-checked and copied inside the seam** (E-05): the kernel's quality wrapper covers the call and read the point's fields outside it, so a `null`, a missing field or a throwing accessor escaped the seam its own attribution names. Throwing from inside `anchorTarget` is what puts the fault back on the `FAILURE_LANDING_TARGET` quality track §Validation already publishes for it |
...
| `onError` | `reportFailure` | Latch read before. **The two kernel-owned routes into it are guarded as well** (E-03): `runAdmission`'s catch and `reportQuality` both reached the behavior's hook unconditionally, so a resolver that called `destroy()` and _then_ threw had its own destruction reported back through a declared callback. The guard is kernel-side because the rule is controller lifetime rather than domain settlement, and both behaviors reach the same two call sites |
...
| **B-6** | Three ways, because the claim has three halves: `tests/consumer.node.test.ts` §`CONSTRAINT` compiles an out-of-line installer against the packed middle tier; `tests/free-drag/feature.declaration.test.ts` states what the type refuses; and `tests/free-drag/lifecycle.browser.test.ts` **runs** one, which is what makes L-3 discriminating |
...
| **L-4** | The same suite §the landing origin, reading `LandingContext.from` through a third-party `startLanding`: axis-locked, clamped, after a `moveTo()`, and from a **release point that differs from the last processed move** by (40, 20) — plus the rendered transform at that instant, which is the half a `release.effect` doing nothing would fail |
