...
| `landing({ duration })` is a function / `>= 0` / is a number | **Measured redundant** — see below | inside `LandingStart` (`:1497`) | `FAILURE_LANDING_CREATE` (10) |
...
**Where the refusal landed, while it existed: `FAILURE_LANDING_CREATE` (10).** The check runs inside the library's `LandingStart`, which the kernel wraps at exactly that stage (`kernel.ts:1487-1497`) — the same slot `animate()`'s own throw for `NaN` already produces. One predicate, no second path.
