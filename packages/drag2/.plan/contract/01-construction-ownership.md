...
**C4-03's argument is intact; D-48 moves the boundary it applies to.** The reasoning is worth keeping in front of the reader, because it is what makes D-48 safe rather than what D-48 overrules. It ran: _the public parameter is the opaque `Behavior<Controller>`, not the factory_ — an earlier version of this section declared `draggable()` against `BehaviorFactory<Controller, Part, Activation>` and had `sortable()` return one, and because this document is normative that was not a stale example. It published the internal factory type _and_ both private generic parameters **at the ordinary public boundary**, and it contradicted §03 §Closed for real.
