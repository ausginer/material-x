...
| [`04-frame-slicing.md`](contract/04-frame-slicing.md) | **Still untouched — now with a header saying so, and why.** D-66 came closest: storing the classifying error for the terminal would have added a part field, an initialiser and a reset obligation. Constructing the result at the failure site avoids all three |
...
**D-66's carrier, because the obvious guess is wrong.** Do **not** reach for `reportFailure(stage, error)` — it is admission-only by contract (_a failure with no operation to settle_) and an earlier draft of D-66 named it in error. The in-operation carrier is **`SettlementInput` with `SETTLED_FAILED`**, which already carries `{ stage, error }`, and the failure checkpoint already opens a settlement with it and runs the ordinary settlement seam stamped `REPORTING`. The change is one branch of the behavior's `settlement.prepare`: on `SETTLED_FAILED` it now stages a `canceled` domain result instead of leaving `domain` null. **`finalized` then publishes `current.domain` unconditionally.**

**Two consequences worth carrying:**

- **[04](contract/04-frame-slicing.md) stays untouched, and deliberately so.** The fallback is constructed at the failure site and written into the `domain` field the part already has. Storing the raw error for later would have needed a new part field, a `createFramePart` initialiser and a `resetFramePart` obligation — i.e. it would have made 04 the third document this revision touches. It does not.
- **The fallback's `CancelStage` is the behavior's to derive**: `AT_CONSUMER` when it has published a `ReorderRequest` that has not resolved, `AT_PROPOSAL` otherwise. The kernel supplies a `CancelStage` only for `SETTLED_CANCELED`; widening `SETTLED_FAILED` to carry one would be an SPI change for information the behavior already holds.
