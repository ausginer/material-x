...
> **~~Re-taken 2026-08-28, and it moved down again: 147 B, 1.38% of `complete`~~ — 10,686 B against baseline A's ~~10,539 B~~, measured at `cdc83990`** (~~142 B~~, ~~10,677~~/~~10,535~~ — F-150: the recorded figures were taken at a tree state between two commits and reproduced at neither). **The premium here is 222 B, 2.08%** — the baseline was stale, not the composed figure; see §The denominator was broken below. The pass was D-147, D-148, D-149 and D-151 together, and **only three of the four can be in it**: D-151 is entirely type-level and erased. What shrank is what left — `AxisSource` and the `TAG_POLICY` staged return (D-148), and both runtime aggregates (D-149), which took `sortable/runtime.js` and `free-drag/runtime.js` out of every graph along with `free-drag/slots.js`, since a module holding only inlinable constants does not survive the bundler. Minified: every sortable row **−400 B**, every free-drag row **−384 B**, `both behaviors` **−786 B**. **Baseline A moved too, −145 B**, which is why the premium falls by less than the composed rows do — it composes the same `src/` modules by hand, so a deletion inside the behavior reaches it as well; the premium is the _difference_, and only the composition plumbing is in it. Controls: `kernel.js` 17,017/6,116 and `drag.js` 344/159, both byte-identical, and baseline B unchanged at 22,573/6,889. **Published declarations grow 92.89 → 97.83 kB**, which is D-151's four shared helpers, two entry aliases and the widened signatures — erased at runtime, and the reason the composed rows fall while the type surface grows.

**~~Amended the same day: 131 B, 1.24%~~ — 10,670 B against baseline A's ~~10,539 B~~; the corrected premium is 206 B, 1.93%** (§The denominator was broken) (~~138 B~~, ~~10,671~~/~~10,533~~ — F-150: those were computed from the previous paragraph's arithmetic rather than re-measured, and the previous paragraph's own figures were the ones that did not reproduce). Both behaviors' module-private `install()` is inlined into the factories that returned it: with the runtime aggregates gone it was a name for two constructor calls, and free drag had one caller. Minified −38 B per sortable row, −25 B per free-drag row, −65 B for `both behaviors`; baseline A moved **0 B** on the repaired fixture (~~+2 B~~, an artifact of the stale one), which is the honest shape of a change that touches only composed code. `BehaviorInstall` stays — it is the kernel tier's published name for what a `BehaviorFactory` returns (D-68), so it never depended on the two internal signatures that named it.

**Re-taken 2026-08-28 after D-152: 208 B, 1.96% of `complete`** — 10,603 B against baseline A's 10,395 B. `SeamRejection`, `isRejection` and both `requestFailure` adapters are deleted, and the eight rejection returns become throws. **This is the first row in the series where `kernel.js` moves**: −152 B minified, −45 B brotli, because the structural probe and the two adapters were kernel code rather than behavior code. Composition rows fall **−207 to −209 B** minified and −57 to −82 B brotli — `free drag complete` is the −209 (23,518 → 23,309) and it was outside the range this paragraph first quoted (F-161) — with −265 B for `both behaviors`. `drag.js` and baseline B stay byte-identical, and the module topology is unchanged at 44 against 44.

**The premium rose 2 B, and that is the honest shape of it.** Composed `complete` falls 67 B brotli and baseline A falls 69 B. A change entirely inside the kernel reaches the hand-composed build slightly _more_ than the composed one, because the composed rows carry a little machinery the deletion does not touch.

### The denominator was broken, and the descending series was mostly its artifact

**F-157, tier A, found in review and repaired here.** `bench/size/noncomposed.js` — baseline A, the denominator of every premium figure in this section — imported `createSortableRuntime` and called the one-argument `createSortableSpec(rt)`. **D-149 deleted both**, at `cdc83990`. The fixture kept building for four commits because the bundler overwrites what it emits and removes nothing: `sortable/runtime.js` stopped being emitted when the aggregate left it, and the previously-emitted file survived on disk, still exporting the deleted function and still importable by relative path. **On a clean checkout of any commit from `cdc83990` on, `just size` did not run at all** — `UNRESOLVED_IMPORT`, before a single byte was measured.

So baseline A bundled a module the library does not ship, and every premium taken against it since `cdc83990` was understated. ~~The old figures are not corrected in place because the arithmetic was never the error~~ — they _are_ corrected in place above, struck rather than rewritten, because each was correct as arithmetic and wrong as a measurement.

Re-measured at every point in the series, from repaired fixtures at commits that exist:

| Commit | Recorded | Baseline A | **Corrected premium** |
| --- | --- | --- | --- |
| `4568e563` (D-146) | 221 B | 30 modules, all live | **221 B** — sound; the fixture was still correct here |
| `cdc83990` (D-149) | 147 B | 30, **one dead** | **222 B** (10,686 − 10,464) |
| `25084a9c` (`install` inline) | 131 B | 30, **one dead** | **206 B** (10,670 − 10,464) |
| `dc2a1d4c` (D-152) | 113 B | 30, **one dead** | **208 B** (10,603 − 10,395) |

**~~221 → 147 → 131 → 113~~ is withdrawn as a series.** Corrected, the premium is **flat at 206–222 B since D-146**: 221 → 222 → 206 → 208. The only real movement in it is the `install()` inline's −16 B. Two sentences that read as findings go with it: _baseline A falls −209 B with the rows, since it hand-composes the same spec modules_ inverted the mechanism — the repaired baseline falls **further** than the composed rows, and the recorded one lagged only because it retained a module they had already lost; and _it moved down again_ described the denominator shrinking, not the plumbing.
...
| **Composition overhead** | 283 B, 2.4% — **221 B since D-146, and 206–222 B at every taking since** (F-157) | The price of the architecture, and **stable** rather than falling: the ~~147 B~~ and ~~113 B~~ readings were a stale denominator, and the one real movement since D-146 is the `install()` inline's −16 B. The thing 03 asked to have weighed rather than removed |
