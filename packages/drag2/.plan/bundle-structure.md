...
> **Re-taken 2026-08-28, and it moved down again: 147 B, 1.38% of `complete`** — 10,686 B against baseline A's 10,539 B, measured at `cdc83990` (~~142 B~~, ~~10,677~~/~~10,535~~ — F-150: the recorded figures were taken at a tree state between two commits and reproduced at neither). The pass was D-147, D-148, D-149 and D-151 together, and **only three of the four can be in it**: D-151 is entirely type-level and erased. What shrank is what left — `AxisSource` and the `TAG_POLICY` staged return (D-148), and both runtime aggregates (D-149), which took `sortable/runtime.js` and `free-drag/runtime.js` out of every graph along with `free-drag/slots.js`, since a module holding only inlinable constants does not survive the bundler. Minified: every sortable row **−400 B**, every free-drag row **−384 B**, `both behaviors` **−786 B**. **Baseline A moved too, −145 B**, which is why the premium falls by less than the composed rows do — it composes the same `src/` modules by hand, so a deletion inside the behavior reaches it as well; the premium is the _difference_, and only the composition plumbing is in it. Controls: `kernel.js` 17,017/6,116 and `drag.js` 344/159, both byte-identical, and baseline B unchanged at 22,573/6,889. **Published declarations grow 92.89 → 97.83 kB**, which is D-151's four shared helpers, two entry aliases and the widened signatures — erased at runtime, and the reason the composed rows fall while the type surface grows.

**Amended the same day: 131 B, 1.24%** — 10,670 B against baseline A's 10,539 B (~~138 B~~, ~~10,671~~/~~10,533~~ — F-150: those were computed from the previous paragraph's arithmetic rather than re-measured, and the previous paragraph's own figures were the ones that did not reproduce). Both behaviors' module-private `install()` is inlined into the factories that returned it: with the runtime aggregates gone it was a name for two constructor calls, and free drag had one caller. Minified −38 B per sortable row, −25 B per free-drag row, −65 B for `both behaviors`; baseline A moved **+2 B**, which is the honest shape of a change that touches only composed code. `BehaviorInstall` stays — it is the kernel tier's published name for what a `BehaviorFactory` returns (D-68), so it never depended on the two internal signatures that named it.

**Re-taken 2026-08-28 after D-152: 113 B, 1.07%** — 10,603 B against baseline A's 10,490 B. `SeamRejection`, `isRejection` and both `requestFailure` adapters are deleted, and the six-plus-two rejection returns become throws. **This is the first row in the series where `kernel.js` moves**: −152 B minified, −45 B brotli, because the structural probe and the two adapters were kernel code rather than behavior code. Every composition row falls −207 to −208 B minified (−265 B for `both behaviors`), and **baseline A falls −209 B** with them, since it hand-composes the same spec modules — so the premium moves by the difference alone. `drag.js` and baseline B stay byte-identical, and the module topology is unchanged at 44 against 44.

**And the previous figure is corrected rather than re-derived** (F-150). The 142 B recorded for the D-147…D-151 pass was taken at a tree state between two commits and matched neither: at `cdc83990` the premium is **147 B** and at `25084a9c` it is **131 B**. The series below quotes takings at commits that exist.

**Read the takings as one series, with the correction F-136 made to the middle one**: 289 B (M-3′) → **289 B** immediately before D-146's arc → 221 B (D-146) → 147 B (D-147…D-151) → 131 B (the `install` inline) → 113 B (D-152). The 283 B figure above was a prior taking quoted as the _from_ of a later one, which is the mistake the finding records.
...
| **Composition overhead** | 283 B, 2.4% — **221 B since D-146; 147 B since D-149; 113 B, 1.1% since D-152** | The price of the architecture, stable across two feature additions and falling twice since: the per-key groups deleted `claim`, and dissolving both runtime aggregates took two modules out of every graph. The thing 03 asked to have weighed rather than removed |
