...
**The five kernel-carrying `control:` rows are removed rather than re-declared**, for the whole D-170 arc series, and owed back at its close as [O-13](../obligations.md). Their last reading before the suspension, against `5f7a901a4`: `free drag minimal` **−23**, `free drag + bounds` **+5**, `free drag + landing` **+2**, `free drag complete` **−1**, `kernel root — kernel.js` **0**. **The two that stay are the result**: `vocabulary root — drag.js` at 142 B and baseline B at 6,889 B, byte-identical on both figures. Re-declaring the five after every kernel-tier pass is what this file calls a budget wearing an exact number, and the restoration is a single dated act at the series' end rather than four.
