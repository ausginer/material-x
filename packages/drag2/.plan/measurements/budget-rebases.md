...
**One number in the table is the implementation choosing the settled reading over the cheaper one.** Answering `prefers-reduced-motion` with _no tail_ instead of a zero-length one is **5 B smaller** on the two rows that install a landing, and it was rejected: the published contract reads the `duration` thunk once per landing **before** the media query, so refusing early would make a consumer's settle-time side effect observable only for users who have not asked for reduced motion. The 5 B is recorded rather than netted off.
...
**The kernel's own row is the cheapest place to read what step 6 cost**, because it carries the class and no behavior: **+252 B** on 5,910. What was added is a `constructor`, an `implements` clause, one `AttemptSlots` literal, and eleven wrapper closures over methods that cross into a receiverless position — seven of them fields, four of those on paths where a per-call closure would have been allocation rather than bytes. What was removed is the seven-member `host` literal and the two-member `Kernel` handle. Every other row lands within 25 B of it, which is what says the change is the kernel's alone and nothing leaked into a behavior. **Re-based 2026-09-04, Stage 0's close — F-328's guard and F-329's message.** Measured against `a92d46ba`. Two source edits, both in files every composition that carries their tier reaches: one `if (!this.#queue.closed)` around the reporting route's terminal in `kernel/kernel.ts`, and six characters out of a diagnostic literal in `free-drag/spec.ts`. **No budget moved and no ceiling was touched** — every row is between 0.06 and 0.17 kB under, exactly as before.

| Row                       | `a92d46ba` | landed | Δ      | budget |
| ------------------------- | ---------- | ------ | ------ | ------ |
| free drag minimal         | 8,116      | 8,111  | **−5** | 8,266  |
| free drag + bounds        | 8,273      | 8,274  | **+1** | 8,423  |
| free drag + landing       | 8,275      | 8,277  | **+2** | 8,425  |
| free drag complete        | 8,426      | 8,423  | **−3** | 8,576  |
| kernel root — `kernel.js` | 6,162      | 6,164  | **+2** | 6,312  |
...
**Re-based 2026-09-05, D-181's Arc B — the frame transaction's extraction and the stamp's deletion.** Measured against `185fb371`, full table and the move-path null result in [`arc-b.md`](arc-b.md). One module enters every graph that carries the kernel at **+359 B minified**, and the compressed bundle **shrinks** on ~~thirteen~~ twelve of the fifteen rows; no budget moved and every **kernel-carrying** row is 0.09 to 0.13 kB under its ceiling. **Both quantifiers corrected 2026-09-07 (F-378, F-381)**: the table has twelve negative rows, one positive and two zero, and over all fifteen the slack runs 0.063 to 0.151 kB.
...
**Not re-based 2026-09-07, Arc B's remediation — D-184's construction-window guards and D-186's deletion of the identity conjunct.** Measured against `5f7a901a4`, full table in [`arc-b-remediation.md`](arc-b-remediation.md). Every kernel-carrying composition pays **+5 B minified** and no module moved; Brotli runs **−30 to +11 B** with no row breaching its ceiling and slack across all fifteen at 0.06–0.15 kB. **No ceiling moves, because §18 re-bases after a shrink and this is not one** — D-186's own deletion would have taken 18 to 44 B off the kernel rows, and D-184's guards land beside it in the same unit.

**The five kernel-carrying `control:` rows are removed rather than re-declared**, for the whole D-170 arc series, and owed back at its close as [O-13](../obligations.md). Their last reading before the suspension, as the **figures** §18 requires rather than deltas against a baseline in another table: `free drag minimal` **8,136**, `free drag + bounds` **8,298**, `free drag + landing` **8,295**, `free drag complete` **8,447**, `kernel root — kernel.js` **6,210** Brotli — respectively −23, +5, +2, −1 and 0 B against the `control:` values then declared, measured against `5f7a901a4`. **The two that stay are the result**: `vocabulary root — drag.js` at 142 B and baseline B at 6,889 B, byte-identical on both figures. Re-declaring the five after every kernel-tier pass is what this file calls a budget wearing an exact number, and the restoration is a single dated act at the series' end rather than four.

**Not re-based 2026-09-08, Arc C — D-189's deletion of the settlement identity slot.** Measured against `06ad3ff87`, full table in [`arc-c.md`](arc-c.md). Every kernel-carrying composition sheds exactly **−136 B minified** and no module moved; Brotli runs **−38 to +1 B**, with no row breaching its ceiling and slack at 0.06–0.16 kB over all fifteen rows and 0.12–0.16 kB over the thirteen the change reaches. **This is a shrink and it still does not re-base**, which is the case §18's third bullet leaves to the reading: re-basing each moved row to its landed figure plus the standing ~150 B would **raise nine of the thirteen ceilings**, lower two and leave two, because the ceilings were last set at D-170 step 6 and the tree has spent what it gained since. A re-base that loosens the instrument after a shrink records what happened instead of checking it.

**Two controls held and the other five are still suspended.** `vocabulary root — drag.js` at 142 B and baseline B at 6,889 B are byte-identical on both figures and gained no module — this pass's declared controls, and the result. The four free-drag rows and `kernel root` carry no `control:` at all: they are omitted for the D-170 arc series and owed back at its close as [O-13](../obligations.md), and Arc C is not that close. **This pass reads their ceilings during the suspension**, which is what O-13's amended clause now says the close does not have a monopoly on. **Not re-based 2026-09-09, Arc D — D-191's membership pass over the sortable's activation record.** Measured against `0c62b02c6`, full table in [`arc-d.md`](arc-d.md). The eight sortable-carrying compositions shed **−264 B minified** (−268 on the two `xy` rows, which is `xy.ts`'s own read moving to the frame) and **45 to 65 B Brotli**; no module entered or left any graph, no row breached its ceiling, and slack runs 0.06–0.22 kB over all fifteen. **This is a shrink, it is the last arc of the series, and it still does not re-base.** The seven rows that did not move have nothing to re-base; the eight that did land 173 to 216 B under ceilings whose standing headroom is ~150 B, so the reclaimable slack is **23 to 66 B** on rows of 10.2 to 12.3 kB. This is the fourth consecutive arc to move the same rows in the same direction, which makes _a ceiling following every arc down stops being a ceiling_ strongest here rather than spent — and [SC-1](../obligations.md)'s triggers are unmet: no row is negative and every byte that moved is attributable to one named landed change. **The series closing is not the reason and was not used as one** (D-187): a boundary may restore an instrument and may never spend one.

**The five suspended controls are restored here, and a restoration is not a re-base.** `free drag minimal` **8,116**, `free drag + bounds` **8,274**, `free drag + landing` **8,276**, `free drag complete` **8,426**, `kernel root — kernel.js` **6,185** — read against the figures the suspension recorded at **8,136 / 8,298 / 8,295 / 8,447 / 6,210**, an interval of **−20, −24, −19, −21 and −25 B** every byte of which is attributable to F-394's deletion and to Arc C's, with the chain continuous at each seam ([`arc-d.md`](arc-d.md) §The controls). ~~fresh exact figures for rows this arc did not move, which is the reading a returning equality wants~~ — **struck (F-409)**: arc-local inertness is not that reading. The restoration is the one act the close of the D-170 arc series authorises ([O-13](../obligations.md), discharged). The two that never left held as well: `vocabulary root — drag.js` at 142 B and baseline B at 6,889 B, byte-identical with no module gained. **Seven `control:` rows again, and no `budget:` moved with them.**

**Not re-based 2026-09-09, Arc D's remediation** — D-194's variance obligation and F-416's release-site snapshot, landed with eight record and instrument repairs. Measured against `9e74f52fa`, full table in [`arc-d-remediation.md`](arc-d-remediation.md). Every sortable-carrying composition pays **−6 B minified** — one identifier, counted once per graph — and Brotli runs **−15 to +8 B** with the two columns disagreeing in direction on five rows; no module moved, no row breached its ceiling, and slack runs 0.06–0.22 kB over all fifteen. **There is no shrink to re-base after**: five rows are positive on Brotli, three negative and seven flat. **All seven `control:` rows are live for the first pass since the restoration and all seven held exactly**, which is the reading O-13's discharge was for.