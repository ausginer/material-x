...
**The five kernel-carrying controls are removed for the D-170 arc series** and booked back at its close as [O-13](../obligations.md), which is F-385's disposition under D-186. `measure.ts`'s own rule omits a control _on a row a pass is expected to move_, and every kernel-tier pass is expected to move all five; re-declaring them after each arc produces a figure read off the result, which is the failure [`budget-rebases.md`](budget-rebases.md) names as _a budget wearing an exact number_. They were still declared at the moment this pass ran, so the **figures** they produced under this pass are the last reading before the suspension, and §18 asks for figures rather than a delta against a baseline held in another table: `free drag minimal` **8,136**, `free drag + bounds` **8,298**, `free drag + landing` **8,295**, `free drag complete` **8,447**, `kernel root — kernel.js` **6,210** Brotli — a movement of −23, +5, +2, −1 and 0 B against the `control:` values then declared. The restoration reads its own figures against these five ([`arc-d.md`](arc-d.md) §The controls, D-192). **What the suspension withdraws is stated at [O-13](../obligations.md)**, which is the one place it is stated; nothing here says what the remaining rows still cover.
...
**Every kernel-carrying row pays exactly −6 B minified and no other row moves at all**, which is one deleted expression arriving in one module and being counted once per graph. Brotli spreads it from **−22 B** to **+12 B**: two conjuncts and their operator are a repeat the compressor was already paying little for, so removing them buys almost nothing and rearranges the window on two free-drag rows. **The +11 on `free drag + landing` and the +12 on `free drag complete` are two readings a live control would have reported** and this paragraph explains instead — `controlViolations()` asserts a Brotli equality in both directions, and both rows were under suspension. They are named as such rather than absorbed (F-411). **Shipped bytes are compressed bytes**, so Brotli is the reported figure and the minified column is what says the change is one thing rather than fifteen.

**Both surviving controls held exactly** — 142 B and 6,889 B, byte-identical, no module gained. They were declared before the pass as rows this change must not reach, and it did not. **No module moved in any composition**, and no ceiling was approached: slack runs 0.06 to 0.15 kB over all fifteen rows — 0.063 kB on `vocabulary root` to 0.151 kB on baseline B — which is the same band, over the same population, as the section above.
