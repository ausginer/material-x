...
> **REPURPOSED 2026-08-29 by D-155**, which settles Q-L2 and supersedes the question this contract was written for. Read [`ql2-presentation-tail-claude.md`](../reviews/phase-23/ql2-presentation-tail-claude.md) §11 first. **Arm A is no longer a hypothetical ceiling — it is D-155's removal floor (A′)**, and a second arm **A″** adds the relinquished tail back so the net is measured rather than subtracted. **Arms B and C are withdrawn**: B conditionalized a capability that no longer exists, and C degeneralized a counter that goes with it. §1's invariants, §3's untouchable set and §4's controls carry over unchanged — except that `armSettlement`'s anchor half and `joinSettlement`'s pin are now untouchable for a stated architectural reason rather than a measurement one. **This measurement is not a gate on D-155.** Amended 2026-08-29: arm **A″** prices retaining `inheritedSpace` plus an **additive-`translate`** runner, which is smaller than the inverse-`transform` shape it was written against.
