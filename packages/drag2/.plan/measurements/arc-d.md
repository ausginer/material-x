...
**The one cost that went the other way is install weight, and it is worth naming** ([`CONTRIBUTING.md`](../../../../CONTRIBUTING.md) §4 (c)). The published declarations went from **116,389 B to 116,432 B raw** — **+43 B** — across the same 33 files, of which comment went from **85,276 B to 85,343 B**, **+67 B**: `snapshot` arriving on `InsertionFrameView` brought a docblock a third-party axis author reads, and the members that left were internal and carried theirs out of an unpublished type. Comment grew by more than the file did, so the declaration text itself shed 24 B. **43 B of tarball against 45 to 65 B off every composition's shipped bytes** — the trade is the right way round, and the figure is recorded rather than inferred.

**Stated in bytes rather than to two decimals, and the precision is the point** (F-425). ~~116.39 kB to 116.43 kB … about 40 B of tarball~~ — at two-decimal-kB a later change of a few dozen bytes cannot move the recorded figure, so any obligation to _record the new figure here_ could be discharged by writing the same number. **This sentence is Arc D's own install-weight cost and stays scoped to it**: the post-Arc-D remediation's declaration-weight delta is a different measurement and is booked in [`arc-d-remediation.md`](arc-d-remediation.md), not appended here.
