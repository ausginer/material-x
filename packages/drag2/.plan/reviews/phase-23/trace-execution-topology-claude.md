...
**F-180 — the profiled bundle's per-module attribution has never been taken, and it answers a question `bench/size` cannot.** Tier C, open. `bench/size` reports Brotli per _composition_; it cannot say which module inside a composition carries the bytes. The embedded source maps do, exactly, for the profiled bundle — §4 is the first such decomposition in the record, and it is what makes ② checkable rather than plausible. `bundle-structure.md` reasons about the shared set from composition marginals; a per-module attribution is a different instrument reaching the same subject, and the two have never been compared.
