...
const CHECKBOX_STATE_MAP: Readonly<Record<string, Param>> = {
...
export function groupCheckboxTokens(tokenName: string): GroupResult | null {
...
export function createCheckboxExtensions(): ExtensionCallback {
  return (m) => {
...
export const checkboxAllowedTokensSelector: GroupSelector =
  createAllowedTokensSelector([
    'container.color',
    'container.shape',
    'container.size',
...
export function createCheckboxScopedDeclarationRenderer(): DeclarationBlockRenderer {
  return (path, declarations) => {
    ...
    const stateParam =
      state === 'default' ? null : (CHECKBOX_STATE_MAP[state] ?? null);
